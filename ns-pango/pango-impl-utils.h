...
/* Some functions for handling NS_PANGO_ATTR_SHAPE */
void _ns_pango_shape_shape (const char       *text,
			 unsigned int      n_chars,
			 NsPangoRectangle   *shape_ink,
			 NsPangoRectangle   *shape_logical,
			 NsPangoGlyphString *glyphs);

void _ns_pango_shape_get_extents (gint              n_chars,
			       NsPangoRectangle   *shape_ink,
			       NsPangoRectangle   *shape_logical,
...
ns_pango_unichar_width (gunichar c)
{
...
ns_pango_utf8_strwidth (const gchar *p)
{
  glong len = 0;
  ...
  while (*p)
    {
      len += ns_pango_unichar_width (g_utf8_get_char (p));
 ...
 * Wrap it here. */
static G_GNUC_UNUSED glong
ns_pango_utf8_strlen (const gchar *p, gssize max)
{
  glong len = 0;
  const gchar *start = p;
...
ns_pango_glyph_string_reverse_range (NsPangoGlyphString *glyphs,
				  int start, int end)
{
  int i, j;
...
ns_pango_is_default_ignorable (gunichar ch)
{
  int plane = ch >> 16;
 ...
 * with nicks if NS_PANGO_SHOW_IGNORABLES is used.
 *
...
ns_pango_get_ignorable (gunichar ch)
{
  int i;
...
static inline G_GNUC_UNUSED const char *
ns_pango_get_ignorable_size (gunichar  ch,
                          int      *rows,
                          int      *cols)
{
  const char *nick;
  ...
  nick = ns_pango_get_ignorable (ch);
  if (nick)
    {
      len = strlen (nick);
      if (len < 4)
...
static inline void
ns_pango_parse_variations (const char            *variations,
...
static inline void
ns_pango_maybe_add_feature (GString    *str,
                         const char *features,
                         const char *feature)
{
  char buf[8] = { 0, };
...
static inline char *
ns_pango_combine_features (NsPangoVariant  variant,
                        const char   *features)
{
  ...
  switch (variant)
    ...
    case NS_PANGO_VARIANT_SMALL_CAPS:
      ns_pango_maybe_add_feature (str, features, "smcp=1");
      break;
    case NS_PANGO_VARIANT_ALL_SMALL_CAPS:
      ns_pango_maybe_add_feature (str, features, "smcp=1");
      ns_pango_maybe_add_feature (str, features, "c2sc=1");
      break;
    case NS_PANGO_VARIANT_PETITE_CAPS:
      ns_pango_maybe_add_feature (str, features, "pcap=1");
      break;
    case NS_PANGO_VARIANT_ALL_PETITE_CAPS:
      ns_pango_maybe_add_feature (str, features, "pcap=1");
      ns_pango_maybe_add_feature (str, features, "c2pc=1");
      break;
    case NS_PANGO_VARIANT_UNICASE:
      ns_pango_maybe_add_feature (str, features, "unic=1");
      break;
    case NS_PANGO_VARIANT_TITLE_CAPS:
      ns_pango_maybe_add_feature (str, features, "titl=1");
      break;
...
ns_pango_features_get_variant (const char *features)
{
  NsPangoVariant variant = NS_PANGO_VARIANT_NORMAL;
  gboolean all_caps = FALSE;
  char **tokens;

  if (!features || !*features)
    return NS_PANGO_VARIANT_NORMAL;
  ...
  for (int i = 0; tokens[i]; i++)
    {
      ...
      if (strcmp (s, "smcp=1") == 0)
        variant = all_caps ? NS_PANGO_VARIANT_ALL_SMALL_CAPS : NS_PANGO_VARIANT_SMALL_CAPS;
        ...
        variant = all_caps ? NS_PANGO_VARIANT_ALL_PETITE_CAPS : NS_PANGO_VARIANT_PETITE_CAPS;
