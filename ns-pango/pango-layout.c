 ...
 * NsPangoLayout:
 *
 * A `NsPangoLayout` structure represents an entire paragraph of text.
 *
 * While complete access to the layout capabilities of Pango is provided
 * using the detailed interfaces for itemization and shaping, using
 * that functionality directly involves writing a fairly large amount
 * of code. `NsPangoLayout` provides a high-level driver for formatting
 * entire paragraphs of text at once. This includes paragraph-level
 * functionality such as line breaking, justification, alignment and
 * ellipsization.
 *
 * A `NsPangoLayout` is initialized with a `NsPangoContext`, UTF-8 string
 ...
 * `NsPangoLayout`. The following image shows adjustable parameters
 ...
 * and simply treat the results of a `NsPangoLayout` as a list of lines.
 */

/**
 * NsPangoLayoutIter:
 *
 * A `NsPangoLayoutIter` can be used to iterate over the visual
 * extents of a `NsPangoLayout`.
 *
 * To obtain a `NsPangoLayoutIter`, use [method@Pango.Layout.get_iter].
 *
 * The `NsPangoLayoutIter` structure is opaque, and has no user-visible fields.
 */

#include "config.h"
#include "pango-glyph.h"                /* For ns_pango_shape() */
 ...
 *
 * See ns_pango_layout_get_item_properties for details.
 */
...
typedef struct _PangoLayoutLinePrivate NsPangoLayoutLinePrivate;
...
static gboolean
check_invalid (NsPangoLayoutIter *iter,
               const char      *loc)
{
  if (iter->line->layout == NULL)
    {
      g_warning ("%s: NsPangoLayout changed since NsPangoLayoutIter was created, iterator invalid", loc);
      return TRUE;
    }
  else
    {
...
static void check_context_changed  (NsPangoLayout *layout);
static void layout_changed  (NsPangoLayout *layout);

static void ns_pango_layout_clear_lines (NsPangoLayout *layout);
static void ns_pango_layout_check_lines (NsPangoLayout *layout);

static NsPangoAttrList *ns_pango_layout_get_effective_attributes (NsPangoLayout *layout);

static NsPangoLayoutLine * ns_pango_layout_line_new         (NsPangoLayout     *layout);
static void              ns_pango_layout_line_postprocess (NsPangoLayoutLine *line,
                                                        ParaBreakState  *state,
                                                        gboolean         wrapped);

static void ns_pango_layout_line_leaked (NsPangoLayoutLine *line);

/* doesn't leak line */
static NsPangoLayoutLine * _ns_pango_layout_iter_get_line (NsPangoLayoutIter *iter);
static NsPangoLayoutRun *  _ns_pango_layout_iter_get_run  (NsPangoLayoutIter *iter);

static void ns_pango_layout_get_item_properties (NsPangoItem      *item,
                                              ItemProperties *properties);

static void ns_pango_layout_get_empty_extents_and_height_at_index (NsPangoLayout    *layout,
                                                                int             index,
                                                                NsPangoRectangle *logical_rect,
                                                                gboolean        apply_line_height,
                                                                int            *height);

static void ns_pango_layout_finalize    (GObject          *object);

G_DEFINE_TYPE (NsPangoLayout, ns_pango_layout, G_TYPE_OBJECT)

static void
ns_pango_layout_init (NsPangoLayout *layout)
{
  layout->serial = 1;
  layout->attrs = NULL;
  ...
  layout->alignment = NS_PANGO_ALIGN_LEFT;
  ...
  layout->wrap = NS_PANGO_WRAP_WORD;
  layout->is_wrapped = FALSE;
  layout->ellipsize = NS_PANGO_ELLIPSIZE_NONE;
  layout->is_ellipsized = FALSE;
}

static void
ns_pango_layout_class_init (NsPangoLayoutClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = ns_pango_layout_finalize;
}

static void
ns_pango_layout_finalize (GObject *object)
{
  NsPangoLayout *layout;

  layout = NS_PANGO_LAYOUT (object);

  ns_pango_layout_clear_lines (layout);
  ...
  if (layout->attrs)
    ns_pango_attr_list_unref (layout->attrs);

  g_free (layout->text);

  if (layout->font_desc)
    ns_pango_font_description_free (layout->font_desc);

  if (layout->tabs)
    ns_pango_tab_array_free (layout->tabs);

  G_OBJECT_CLASS (ns_pango_layout_parent_class)->finalize (object);
}
 ...
 *
 * Create a new `NsPangoLayout` object with attributes initialized to
 * default values for a particular `NsPangoContext`.
 *
 * Returns: (transfer full): the newly allocated `NsPangoLayout`
 */
NsPangoLayout *
ns_pango_layout_new (NsPangoContext *context)
{
  NsPangoLayout *layout;

  g_return_val_if_fail (context != NULL, NULL);

  layout = g_object_new (NS_TYPE_PANGO_LAYOUT, NULL);

  layout->context = context;
  layout->context_serial = ns_pango_context_get_serial (context);
 ...
 */
NsPangoLayout*
ns_pango_layout_copy (NsPangoLayout *src)
{
  NsPangoLayout *layout;

  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (src), NULL);

  /* Copy referenced members */

  layout = ns_pango_layout_new (src->context);
  if (src->attrs)
    layout->attrs = ns_pango_attr_list_copy (src->attrs);
  if (src->font_desc)
    layout->font_desc = ns_pango_font_description_copy (src->font_desc);
  if (src->tabs)
    layout->tabs = ns_pango_tab_array_copy (src->tabs);
  ...
  memcpy (&layout->copy_begin, &src->copy_begin,
          G_STRUCT_OFFSET (NsPangoLayout, copy_end) - G_STRUCT_OFFSET (NsPangoLayout, copy_begin));

  return layout;
}

/**
 * ns_pango_layout_get_context:
 * @layout: a `NsPangoLayout`
 *
 * Retrieves the `NsPangoContext` used for this layout.
 *
 * Returns: (transfer none): the `NsPangoContext` for the layout
 */
NsPangoContext *
ns_pango_layout_get_context (NsPangoLayout *layout)
{
...
/**
 * ns_pango_layout_set_width:
 * @layout: a `NsPangoLayout`.
 * @width: the desired width in Pango units, or -1 to indicate that no
 *   wrapping or ellipsization should be performed.
 *
 * Sets the width to which the lines of the `NsPangoLayout` should wrap or
 * get ellipsized.
 *
 * The default value is -1: no width set.
 */
void
ns_pango_layout_set_width (NsPangoLayout *layout,
                        int          width)
{
...
/**
 * ns_pango_layout_get_width:
 * @layout: a `NsPangoLayout`
 *
 * Gets the width to which the lines of the `NsPangoLayout` should wrap.
 *
 * Returns: the width in Pango units, or -1 if no width set.
 */
int
ns_pango_layout_get_width (NsPangoLayout    *layout)
{
...
/**
 * ns_pango_layout_set_height:
 * @layout: a `NsPangoLayout`.
 * @height: the desired height of the layout in Pango units if positive,
 *   or desired number of lines if negative.
 *
 * Sets the height to which the `NsPangoLayout` should be ellipsized at.
 *
 ...
 * @layout and ellipsization mode of @layout is not %NS_PANGO_ELLIPSIZE_NONE.
 * The behavior is undefined if a height other than -1 is set and
 * ellipsization mode is set to %NS_PANGO_ELLIPSIZE_NONE, and may change in the
 * future.
 *
 * Since: 1.20
 */
void
ns_pango_layout_set_height (NsPangoLayout *layout,
                         int          height)
{
  g_return_if_fail (layout != NULL);
 ...
 */
int
ns_pango_layout_get_height (NsPangoLayout *layout)
{
  g_return_val_if_fail (layout != NULL, 0);
 ...
 * set the width to -1.
 *
 * The default value is %NS_PANGO_WRAP_WORD.
 */
void
ns_pango_layout_set_wrap (NsPangoLayout   *layout,
 ...
 */
NsPangoWrapMode
ns_pango_layout_get_wrap (NsPangoLayout *layout)
{
  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (layout), 0);

  return layout->wrap;
}

/**
 * ns_pango_layout_is_wrapped:
 * @layout: a `NsPangoLayout`
 *
 ...
 */
gboolean
ns_pango_layout_is_wrapped (NsPangoLayout *layout)
{
  g_return_val_if_fail (layout != NULL, FALSE);

  ns_pango_layout_check_lines (layout);

  return layout->is_wrapped;
}

/**
 * ns_pango_layout_set_indent:
 * @layout: a `NsPangoLayout`
 ...
 */
void
ns_pango_layout_set_indent (NsPangoLayout *layout,
                         int          indent)
{
  g_return_if_fail (layout != NULL);
 ...
 */
int
ns_pango_layout_get_indent (NsPangoLayout *layout)
{
  g_return_val_if_fail (layout != NULL, 0);
 ...
 */
void
ns_pango_layout_set_spacing (NsPangoLayout *layout,
                          int          spacing)
{
  g_return_if_fail (layout != NULL);
 ...
 */
int
ns_pango_layout_get_spacing (NsPangoLayout *layout)
{
  g_return_val_if_fail (layout != NULL, 0);
...
/**
 * ns_pango_layout_set_line_spacing:
 * @layout: a `NsPangoLayout`
 ...
 */
void
ns_pango_layout_set_line_spacing (NsPangoLayout *layout,
                               float        factor)
{
  g_return_if_fail (layout != NULL);
...
/**
 * ns_pango_layout_get_line_spacing:
 * @layout: a `NsPangoLayout`
 *
 ...
 */
float
ns_pango_layout_get_line_spacing (NsPangoLayout *layout)
{
...
/**
 * ns_pango_layout_set_attributes:
 * @layout: a `NsPangoLayout`
 * @attrs: (nullable) (transfer none): a `NsPangoAttrList`
 *
 ...
 */
void
ns_pango_layout_set_attributes (NsPangoLayout   *layout,
                             NsPangoAttrList *attrs)
{
  NsPangoAttrList *old_attrs;
  ...
  if (layout->attrs &&
      ns_pango_attr_list_equal (layout->attrs, attrs))
    return;
  ...
  layout->attrs = attrs;
  if (layout->attrs)
    ns_pango_attr_list_ref (layout->attrs);

  g_clear_pointer (&layout->log_attrs, g_free);
  layout_changed (layout);

  if (old_attrs)
    ns_pango_attr_list_unref (old_attrs);
  layout->tab_width = -1;
}

/**
 * ns_pango_layout_get_attributes:
 * @layout: a `NsPangoLayout`
 *
 * Gets the attribute list for the layout, if any.
 *
 * Returns: (transfer none) (nullable): a `NsPangoAttrList`
 */
NsPangoAttrList*
ns_pango_layout_get_attributes (NsPangoLayout *layout)
{
...
/**
 * ns_pango_layout_set_font_description:
 * @layout: a `NsPangoLayout`
 * @desc: (nullable): the new `NsPangoFontDescription`
 ...
 */
void
ns_pango_layout_set_font_description (NsPangoLayout                *layout,
                                   const NsPangoFontDescription *desc)
{
  g_return_if_fail (layout != NULL);

  if (desc != layout->font_desc &&
      (!desc || !layout->font_desc || !ns_pango_font_description_equal(desc, layout->font_desc)))
    {
      if (layout->font_desc)
        ns_pango_font_description_free (layout->font_desc);

      layout->font_desc = desc ? ns_pango_font_description_copy (desc) : NULL;
...
/**
 * ns_pango_layout_get_font_description:
 * @layout: a `NsPangoLayout`
 *
...
ns_pango_layout_get_font_description (NsPangoLayout *layout)
{
  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (layout), NULL);
 ...
 */
void
ns_pango_layout_set_justify (NsPangoLayout *layout,
                          gboolean     justify)
{
  g_return_if_fail (layout != NULL);
 ...
 */
gboolean
ns_pango_layout_get_justify (NsPangoLayout *layout)
{
  g_return_val_if_fail (layout != NULL, FALSE);
...
/**
 * ns_pango_layout_set_justify_last_line:
 * @layout: a `NsPangoLayout`
 ...
 */
void
ns_pango_layout_set_justify_last_line (NsPangoLayout *layout,
                                    gboolean     justify)
{
  g_return_if_fail (layout != NULL);
...
/**
 * ns_pango_layout_get_justify_last_line:
 * @layout: a `NsPangoLayout`
 *
 ...
 */
gboolean
ns_pango_layout_get_justify_last_line (NsPangoLayout *layout)
{
  g_return_val_if_fail (layout != NULL, FALSE);
 ...
 * `NsPangoContext`. (See [method@Pango.Context.set_base_dir]).
 *
 * When the auto-computed direction of a paragraph differs from the
 * base direction of the context, the interpretation of
 * %NS_PANGO_ALIGN_LEFT and %NS_PANGO_ALIGN_RIGHT are swapped.
 *
 * Since: 1.4
 */
void
ns_pango_layout_set_auto_dir (NsPangoLayout *layout,
                           gboolean     auto_dir)
{
  g_return_if_fail (NS_PANGO_IS_LAYOUT (layout));
 ...
 */
gboolean
ns_pango_layout_get_auto_dir (NsPangoLayout *layout)
{
  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (layout), TRUE);

  return layout->auto_dir;
}

/**
 * ns_pango_layout_set_alignment:
 * @layout: a `NsPangoLayout`
 ...
 *
 * The default alignment is %NS_PANGO_ALIGN_LEFT.
 */
void
ns_pango_layout_set_alignment (NsPangoLayout   *layout,
 ...
 */
NsPangoAlignment
ns_pango_layout_get_alignment (NsPangoLayout *layout)
{
  g_return_val_if_fail (layout != NULL, NS_PANGO_ALIGN_LEFT);
  return layout->alignment;
}


/**
 * ns_pango_layout_set_tabs:
 * @layout: a `NsPangoLayout`
 * @tabs: (nullable): a `NsPangoTabArray`
 *
 * Sets the tabs to use for @layout, overriding the default tabs.
 *
 * `NsPangoLayout` will place content at the next tab position
 ...
 */
void
ns_pango_layout_set_tabs (NsPangoLayout   *layout,
                       NsPangoTabArray *tabs)
{
  ...
  if (tabs != layout->tabs)
    {
      g_clear_pointer (&layout->tabs, ns_pango_tab_array_free);

      if (tabs)
        {
          layout->tabs = ns_pango_tab_array_copy (tabs);
          ns_pango_tab_array_sort (layout->tabs);
        }
...
/**
 * ns_pango_layout_get_tabs:
 * @layout: a `NsPangoLayout`
 *
 * Gets the current `NsPangoTabArray` used by this layout.
 *
 * If no `NsPangoTabArray` has been set, then the default tabs are
 ...
 */
NsPangoTabArray*
ns_pango_layout_get_tabs (NsPangoLayout *layout)
{
  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (layout), NULL);

  if (layout->tabs)
    return ns_pango_tab_array_copy (layout->tabs);
  else
    return NULL;
}

/**
 * ns_pango_layout_set_single_paragraph_mode:
 * @layout: a `NsPangoLayout`
 * @setting: new setting
 *
 ...
 */
void
ns_pango_layout_set_single_paragraph_mode (NsPangoLayout *layout,
                                        gboolean     setting)
{
  g_return_if_fail (NS_PANGO_IS_LAYOUT (layout));
...
/**
 * ns_pango_layout_get_single_paragraph_mode:
 * @layout: a `NsPangoLayout`
 *
 ...
 */
gboolean
ns_pango_layout_get_single_paragraph_mode (NsPangoLayout *layout)
{
 ...
 *
 * The default value is %NS_PANGO_ELLIPSIZE_NONE.
 *
 * See [method@Pango.Layout.set_height] for details.
 *
 * Since: 1.6
 */
void
ns_pango_layout_set_ellipsize (NsPangoLayout        *layout,
...
ns_pango_layout_get_ellipsize (NsPangoLayout *layout)
{
  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (layout), NS_PANGO_ELLIPSIZE_NONE);

  return layout->ellipsize;
}

/**
 * ns_pango_layout_is_ellipsized:
 * @layout: a `NsPangoLayout`
 *
 * Queries whether the layout had to ellipsize any paragraphs.
 *
 * This returns %TRUE if the ellipsization mode for @layout
 * is not %NS_PANGO_ELLIPSIZE_NONE, a positive width is set on @layout,
 ...
 */
gboolean
ns_pango_layout_is_ellipsized (NsPangoLayout *layout)
{
  g_return_val_if_fail (layout != NULL, FALSE);
 ...
 */
void
ns_pango_layout_set_text (NsPangoLayout *layout,
                       const char  *text,
                       int          length)
{
  ...
  for (;;) {
     ...
     * ((NsPangoGlyph) -1) by NS_PANGO_GET_UNKNOWN_GLYPH(-1),
     * and that's NS_PANGO_GLYPH_INVALID_INPUT.
     */
    if (!valid)
      *end++ = -1;
  ...
  if (start != layout->text)
    ...
    g_warning ("Invalid UTF-8 string passed to ns_pango_layout_set_text()");

  layout->n_chars = ns_pango_utf8_strlen (layout->text, -1);
 ...
 */
const char*
ns_pango_layout_get_text (NsPangoLayout *layout)
{
  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (layout), NULL);
...
/**
 * ns_pango_layout_get_character_count:
 * @layout: a `NsPangoLayout`
 *
 ...
 */
gint
ns_pango_layout_get_character_count (NsPangoLayout *layout)
{
  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (layout), 0);
 ...
 *
 * See [Pango Markup](ns_pango_markup.html)).
 *
 ...
 */
void
ns_pango_layout_set_markup (NsPangoLayout *layout,
                         const char  *markup,
                         int          length)
{
  ns_pango_layout_set_markup_with_accel (layout, markup, length, 0, NULL);
}

/**
 * ns_pango_layout_set_markup_with_accel:
 * @layout: a `NsPangoLayout`
 * @markup: marked-up text (see [Pango Markup](ns_pango_markup.html))
 * @length: length of marked-up text in bytes, or -1 if @markup is
 *   `NUL`-terminated
 ...
 * as an accelerator will receive a %NS_PANGO_UNDERLINE_LOW attribute,
 * and the first character so marked will be returned in @accel_char.
 * Two @accel_marker characters following each other produce a single
 * literal @accel_marker character.
 */
void
ns_pango_layout_set_markup_with_accel (NsPangoLayout *layout,
                                    const char  *markup,
                                    int          length,
                                    gunichar     accel_marker,
                                    gunichar    *accel_char)
{
  NsPangoAttrList *list = NULL;
  char *text = NULL;
  GError *error;
  ...
  error = NULL;
  if (!ns_pango_parse_markup (markup, length,
                           accel_marker,
                           &list, &text,
                           accel_char,
                           &error))
    {
      g_warning ("ns_pango_layout_set_markup_with_accel: %s", error->message);
      g_error_free (error);
      return;
    }
...
/**
 * ns_pango_layout_get_unknown_glyphs_count:
 * @layout: a `NsPangoLayout`
 *
 ...
 * used in combination with %NS_PANGO_ATTR_FALLBACK, to check if a
 ...
 */
int
ns_pango_layout_get_unknown_glyphs_count (NsPangoLayout *layout)
{
    NsPangoLayoutLine *line;
        ...
        while (runs_list)
          {
            ...
            for (i = 0; i < run->glyphs->num_glyphs; i++)
              {
                if (run->glyphs->glyphs[i].glyph & NS_PANGO_GLYPH_UNKNOWN_FLAG)
                    count++;
              }
...
static void
check_context_changed (NsPangoLayout *layout)
{
  guint old_serial = layout->context_serial;

  layout->context_serial = ns_pango_context_get_serial (layout->context);

  if (old_serial != layout->context_serial)
    ns_pango_layout_context_changed (layout);
}

static void
layout_changed (NsPangoLayout *layout)
{
  layout->serial++;
  if (layout->serial == 0)
    layout->serial++;
...
/**
 * ns_pango_layout_context_changed:
 * @layout: a `NsPangoLayout`
 *
 * Forces recomputation of any state in the `NsPangoLayout` that
 ...
 */
void
ns_pango_layout_context_changed (NsPangoLayout *layout)
{
  g_return_if_fail (NS_PANGO_IS_LAYOUT (layout));
 ...
 * changed using any of the setter functions, or the `NsPangoContext` it
 * uses has changed. The serial may wrap, but will never have the value 0.
 * Since it can wrap, never compare it with "less than", always use "not equals".
 *
 * This can be used to automatically detect changes to a `NsPangoLayout`,
 ...
 */
guint
ns_pango_layout_get_serial (NsPangoLayout *layout)
{
 ...
 */
void
ns_pango_layout_get_log_attrs (NsPangoLayout   *layout,
                            ...
                            gint          *n_attrs)
{
  ...
  if (attrs)
    {
      *attrs = g_new (NsPangoLogAttr, layout->n_chars + 1);
      memcpy (*attrs, layout->log_attrs, sizeof(NsPangoLogAttr) * (layout->n_chars + 1));
    }
...
/**
 * ns_pango_layout_get_log_attrs_readonly:
 * @layout: a `NsPangoLayout`
 ...
 * Since: 1.30
 */
const NsPangoLogAttr *
ns_pango_layout_get_log_attrs_readonly (NsPangoLayout *layout,
                                     gint        *n_attrs)
{
  if (n_attrs)
    *n_attrs = 0;
...
/**
 * ns_pango_layout_get_line_count:
 * @layout: `NsPangoLayout`
 *
 ...
 */
int
ns_pango_layout_get_line_count (NsPangoLayout   *layout)
{
  g_return_val_if_fail (layout != NULL, 0);
 ...
 *   `NsPangoLayout` and must be used with care. It will become invalid on any
 *   change to the layout's text or properties.
 */
GSList *
ns_pango_layout_get_lines (NsPangoLayout *layout)
{
  ns_pango_layout_check_lines (layout);
  ...
  if (layout->lines)
    {
      ...
      while (tmp_list)
        {
          NsPangoLayoutLine *line = tmp_list->data;
          tmp_list = tmp_list->next;

          ns_pango_layout_line_leaked (line);
        }
    }
...
/**
 * ns_pango_layout_get_lines_readonly:
 * @layout: a `NsPangoLayout`
 *
 * Returns the lines of the @layout as a list.
 *
 ...
 */
GSList *
ns_pango_layout_get_lines_readonly (NsPangoLayout *layout)
{
  ns_pango_layout_check_lines (layout);
...
/**
 * ns_pango_layout_get_line:
 * @layout: a `NsPangoLayout`
 * @line: the index of a line, which must be between 0 and
 *   `ns_pango_layout_get_line_count(layout) - 1`, inclusive.
 *
 * Retrieves a particular line from a `NsPangoLayout`.
 *
 * Use the faster [method@Pango.Layout.get_line_readonly] if you do not
 * plan to modify the contents of the line (glyphs, glyph widths, etc.).
 *
 * Returns: (transfer none) (nullable): the requested `NsPangoLayoutLine`,
 ...
 */
NsPangoLayoutLine *
ns_pango_layout_get_line (NsPangoLayout *layout,
                       int          line)
{
  GSList *list_item;
  g_return_val_if_fail (layout != NULL, NULL);
  ...
  if (list_item)
    {
      NsPangoLayoutLine *line = list_item->data;
...
/**
 * ns_pango_layout_get_line_readonly:
 * @layout: a `NsPangoLayout`
 * @line: the index of a line, which must be between 0 and
 *   `ns_pango_layout_get_line_count(layout) - 1`, inclusive.
 *
 ...
 *   or %NULL if the index is out of range. This layout line can be ref'ed
 *   and retained, but will become invalid if changes are made to the
 *   `NsPangoLayout`. No changes should be made to the line.
 *
 * Since: 1.16
 */
NsPangoLayoutLine *
ns_pango_layout_get_line_readonly (NsPangoLayout *layout,
                                int          line)
{
  GSList *list_item;
  g_return_val_if_fail (layout != NULL, NULL);
...
/**
 * ns_pango_layout_line_index_to_x:
 ...
 */
void
ns_pango_layout_line_index_to_x (NsPangoLayoutLine *line,
                              int              index,
                              int              trailing,
                              int             *x_pos)
{
  NsPangoLayout *layout = line->layout;
  GSList *run_list = line->runs;
  int width = 0;

  while (run_list)
    {
      NsPangoLayoutRun *run = run_list->data;
      ...
      if (run->item->offset <= index && run->item->offset + run->item->length > index)
        {
           ...
           */
          g_assert (run->item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_HAS_CHAR_OFFSET);
          attr_offset = ((NsPangoItemPrivate *)run->item)->char_offset;

          ns_pango_glyph_string_index_to_x_full (run->glyphs,
      ...
      width += ns_pango_glyph_string_get_width (run->glyphs);
...
static NsPangoLayoutLine *
ns_pango_layout_index_to_line (NsPangoLayout      *layout,
                            int               index,
                            int              *line_nr,
                            NsPangoLayoutLine **line_before,
                            NsPangoLayoutLine **line_after)
{
  GSList *tmp_list;
  GSList *line_list;
  NsPangoLayoutLine *line = NULL;
  NsPangoLayoutLine *prev_line = NULL;
  int i = -1;

  line_list = tmp_list = layout->lines;
  while (tmp_list)
    {
      NsPangoLayoutLine *tmp_line = tmp_list->data;
...
static NsPangoLayoutLine *
ns_pango_layout_index_to_line_and_extents (NsPangoLayout     *layout,
                                        int              index,
                                        NsPangoRectangle  *line_rect,
                                        NsPangoRectangle  *run_rect)
{
  ...
  if (!ITER_IS_INVALID (&iter))
    while (TRUE)
      {
        NsPangoLayoutLine *tmp_line = _ns_pango_layout_iter_get_line (&iter);
        ...
        ns_pango_layout_iter_get_line_extents (&iter, NULL, line_rect);

        if (!iter.line_list_link->next ||
            ((NsPangoLayoutLine *)iter.line_list_link->next->data)->start_index > index)
          {
            if (run_rect)
              {
                while (TRUE)
                  {
                    NsPangoLayoutRun *run = _ns_pango_layout_iter_get_run (&iter);

                    ns_pango_layout_iter_get_run_extents (&iter, NULL, run_rect);
                    ...
                    if (!ns_pango_layout_iter_next_run (&iter))
                      break;
                  }
              }
        ...
        if (!ns_pango_layout_iter_next_line (&iter))
...
/**
 * ns_pango_layout_index_to_line_x:
 * @layout: a `NsPangoLayout`
 ...
 *   between 0 and ns_pango_layout_get_line_count(layout) - 1)
 * @x_pos: (out) (optional): location to store resulting position within line
 *   (%NS_PANGO_SCALE units per device unit)
 *
 ...
 */
void
ns_pango_layout_index_to_line_x (NsPangoLayout *layout,
                              int          index,
                              gboolean     trailing,
                              int         *line,
                              int         *x_pos)
{
  int line_num;
  NsPangoLayoutLine *layout_line = NULL;
  ...
  layout_line = ns_pango_layout_index_to_line (layout, index,
                                            &line_num, NULL, NULL);
  ...
  if (layout_line)
    {
      ...
      ns_pango_layout_line_index_to_x (layout_line, index, trailing, x_pos);
    }
  else
    {
      if (line)
...
static void
ns_pango_layout_line_get_cursors (NsPangoLayoutLine *line,
                               gboolean         strong,
                               GArray          *cursors)
{
  NsPangoLayout *layout = line->layout;
  int line_no;
  NsPangoLayoutLine *line2;
  ...
  ns_pango_layout_index_to_line_x (layout, line->start_index + line->length, 0, &line_no, NULL);
  line2 = ns_pango_layout_get_line (layout, line_no);
  if (line2 == line)
    end++;
      ...
      if (layout->log_attrs[j].is_cursor_position)
        {
          ...
          ns_pango_layout_get_cursor_pos (layout, p - layout->text,
                                       strong ? &pos : NULL,
                                       strong ? NULL : &pos);
...
/**
 * ns_pango_layout_move_cursor_visually:
 * @layout: a `NsPangoLayout`
 ...
 */
void
ns_pango_layout_move_cursor_visually (NsPangoLayout *layout,
                                   gboolean     strong,
                                   int          old_index,
                                   int          old_trailing,
                                   int          direction,
...
{
  NsPangoLayoutLine *line = NULL;
  NsPangoLayoutLine *prev_line;
  NsPangoLayoutLine *next_line;
  GArray *cursors;
  int n_vis;
  int vis_pos;
  int start_offset;
  ...
  line = ns_pango_layout_index_to_line (layout, old_index, NULL, &prev_line, &next_line);
  ...
  ns_pango_layout_line_get_cursors (line, strong, cursors);

  ns_pango_layout_get_cursor_pos (layout, old_index, strong ? &pos : NULL, strong ? NULL : &pos);
  ...
  if (vis_pos == -1 &&
      old_index == line->start_index + line->length)
    {
      if (line->resolved_dir == NS_PANGO_DIRECTION_LTR)
        vis_pos = cursors->len;
      else
        vis_pos = 0;
    }
  ...
  if (*new_index == line->start_index + line->length && line->length > 0)
    {
      ...
      log_pos = start_offset + ns_pango_utf8_strlen (layout->text + line->start_index, line->length);
      do
        {
          log_pos--;
 ...
 */
gboolean
ns_pango_layout_xy_to_index (NsPangoLayout *layout,
                          int          x,
                          int          y,
                          int         *index,
                          gint        *trailing)
{
  ...
  do
    {
      NsPangoRectangle line_logical;
      int first_y, last_y;

      g_assert (!ITER_IS_INVALID (&iter));

      ns_pango_layout_iter_get_line_extents (&iter, NULL, &line_logical);
      ns_pango_layout_iter_get_line_yrange (&iter, &first_y, &last_y);
      ...
      if (y < first_y)
        {
          if (prev_line && y < (prev_last + (first_y - prev_last) / 2))
          ...
          else
            {
              ...
              found = _ns_pango_layout_iter_get_line (&iter);
      ...
      prev_line = _ns_pango_layout_iter_get_line (&iter);
      prev_last = last_y;
  ...
  retval = ns_pango_layout_line_x_to_index (found,
                                         found_line_x,
                                         index, trailing);
...
/**
 * ns_pango_layout_index_to_pos:
 * @layout: a `NsPangoLayout`
 * @index_: byte index within @layout
 * @pos: (out): rectangle in which to store the position of the grapheme
 *
 * Converts from an index within a `NsPangoLayout` to the onscreen position
 ...
 */
void
ns_pango_layout_index_to_pos (NsPangoLayout    *layout,
                           int             index,
                           NsPangoRectangle *pos)
{
  NsPangoRectangle line_logical_rect = { 0, };
  NsPangoRectangle run_logical_rect = { 0, };
  NsPangoLayoutIter iter;
  NsPangoLayoutLine *layout_line = NULL;
  int x_pos;
  ...
  if (!ITER_IS_INVALID (&iter))
    {
      while (TRUE)
        {
          ...
          ns_pango_layout_iter_get_line_extents (&iter, NULL, &line_logical_rect);
              ...
              do
                {
                  ...
                  ns_pango_layout_iter_get_run_extents (&iter, NULL, &run_logical_rect);
               ...
               while (ns_pango_layout_iter_next_run (&iter));
      ...
      ns_pango_layout_line_index_to_x (layout_line, index, 0, &x_pos);
      pos->x = line_logical_rect.x + x_pos;

      if (index < layout_line->start_index + layout_line->length)
        {
          ns_pango_layout_line_index_to_x (layout_line, index, 1, &x_pos);
...
static NsPangoLayoutRun *
ns_pango_layout_line_get_run (NsPangoLayoutLine *line,
                           int              index)
{
  GSList *run_list;
...
static int
ns_pango_layout_line_get_char_level (NsPangoLayoutLine *line,
                                  int              index)
{
  NsPangoLayoutRun *run;

  run = ns_pango_layout_line_get_run (line, index);
...
static NsPangoDirection
ns_pango_layout_line_get_char_direction (NsPangoLayoutLine *layout_line,
                                      int              index)
{
   return ns_pango_layout_line_get_char_level (layout_line, index) % 2
          ? NS_PANGO_DIRECTION_RTL
          : NS_PANGO_DIRECTION_LTR;
}

/**
 * ns_pango_layout_get_direction:
 * @layout: a `NsPangoLayout`
 ...
 */
NsPangoDirection
ns_pango_layout_get_direction (NsPangoLayout *layout,
                            int          index)
{
  NsPangoLayoutLine *line;

  line = ns_pango_layout_index_to_line_and_extents (layout, index, NULL, NULL);

  if (line)
    return ns_pango_layout_line_get_char_direction (line, index);

  return NS_PANGO_DIRECTION_LTR;
}

/**
 * ns_pango_layout_get_cursor_pos:
 * @layout: a `NsPangoLayout`
 ...
 */
void
ns_pango_layout_get_cursor_pos (NsPangoLayout    *layout,
                             int             index,
                             NsPangoRectangle *strong_pos,
                             NsPangoRectangle *weak_pos)
{
  NsPangoDirection dir1, dir2;
  int level1, level2;
  NsPangoRectangle line_rect = { 666, };
  NsPangoRectangle run_rect = { 666, };
  NsPangoLayoutLine *layout_line = NULL; /* Quiet GCC */
  int x1_trailing;
  int x2;
  ...
  layout_line = ns_pango_layout_index_to_line_and_extents (layout, index,
                                                        &line_rect, &run_rect);
  ...
  if (index == layout_line->start_index)
    {
      ...
      level1 = dir1 == NS_PANGO_DIRECTION_LTR ? 0 : 1;
      if (layout_line->resolved_dir == NS_PANGO_DIRECTION_LTR)
        x1_trailing = 0;
      else
  ...
  else
    {
      ...
      level1 = ns_pango_layout_line_get_char_level (layout_line, prev_index);
      dir1 = level1 % 2 ? NS_PANGO_DIRECTION_RTL : NS_PANGO_DIRECTION_LTR;
      ns_pango_layout_line_index_to_x (layout_line, prev_index, TRUE, &x1_trailing);
    }
  ...
  if (index >= layout_line->start_index + layout_line->length)
    {
      ...
      level2 = dir2 == NS_PANGO_DIRECTION_LTR ? 0 : 1;
      if (layout_line->resolved_dir == NS_PANGO_DIRECTION_LTR)
        x2 = line_rect.width;
      else
        x2 = 0;
    ...
    {
      ns_pango_layout_line_index_to_x (layout_line, index, FALSE, &x2);
      level2 = ns_pango_layout_line_get_char_level (layout_line, index);
      dir2 = level2 % 2 ? NS_PANGO_DIRECTION_RTL : NS_PANGO_DIRECTION_LTR;
    }
 ...
 */
void
ns_pango_layout_get_caret_pos (NsPangoLayout    *layout,
                            int             index,
                            NsPangoRectangle *strong_pos,
                            NsPangoRectangle *weak_pos)
{
  ...
  ns_pango_layout_get_cursor_pos (layout, index, strong_pos, weak_pos);

  /* FIXME: not very efficient to re-iterate here */
  line = ns_pango_layout_index_to_line_and_extents (layout, index, NULL, NULL);
  run = ns_pango_layout_line_get_run (line, index);
  if (!run)
    run = ns_pango_layout_line_get_run (line, index - 1);

  if (!run)
    return;

  hb_font = ns_pango_font_get_hb_font (run->item->analysis.font);
...
static inline int
direction_simple (NsPangoDirection d)
{
  switch (d)
    {
    case NS_PANGO_DIRECTION_LTR :
    case NS_PANGO_DIRECTION_WEAK_LTR :
    case NS_PANGO_DIRECTION_TTB_RTL :
      return 1;
    case NS_PANGO_DIRECTION_RTL :
    case NS_PANGO_DIRECTION_WEAK_RTL :
    case NS_PANGO_DIRECTION_TTB_LTR :
      return -1;
    case NS_PANGO_DIRECTION_NEUTRAL :
      return 0;
    default:
      break;
    }
...
static NsPangoAlignment
get_alignment (NsPangoLayout     *layout,
               NsPangoLayoutLine *line)
{
  NsPangoAlignment alignment = layout->alignment;

  if (alignment != NS_PANGO_ALIGN_CENTER && line->layout->auto_dir &&
      direction_simple (line->resolved_dir) ==
      -direction_simple (ns_pango_context_get_base_dir (layout->context)))
    {
      if (alignment == NS_PANGO_ALIGN_LEFT)
        alignment = NS_PANGO_ALIGN_RIGHT;
      else if (alignment == NS_PANGO_ALIGN_RIGHT)
        alignment = NS_PANGO_ALIGN_LEFT;
    }
...
static void
get_x_offset (NsPangoLayout     *layout,
              NsPangoLayoutLine *line,
              int              layout_width,
              int              line_width,
              int             *x_offset)
{
  NsPangoAlignment alignment = get_alignment (layout, line);
  ...
  else if (alignment == NS_PANGO_ALIGN_CENTER) {
    *x_offset = (layout_width - line_width) / 2;
    /* hinting */
    if (((layout_width | line_width) & (NS_PANGO_SCALE - 1)) == 0)
      {
        *x_offset = NS_PANGO_UNITS_ROUND (*x_offset);
      }
  } else
    *x_offset = 0;
  ...
  if (alignment == NS_PANGO_ALIGN_CENTER)
    return;
...
static void
ns_pango_layout_line_get_extents_and_height (NsPangoLayoutLine *line,
                                          NsPangoRectangle *ink,
                                          NsPangoRectangle *logical,
                                          int            *height);
static void
get_line_extents_layout_coords (NsPangoLayout     *layout,
                                NsPangoLayoutLine *line,
                                int              layout_width,
                                int              y_offset,
                                int             *baseline,
                                NsPangoRectangle  *line_ink_layout,
                                NsPangoRectangle  *line_logical_layout)
{
  int x_offset;
  /* Line extents in line coords (origin at line baseline) */
  NsPangoRectangle line_ink;
  NsPangoRectangle line_logical;
  gboolean first_line;
  int new_baseline;
  int height;
  ...
  ns_pango_layout_line_get_extents_and_height (line, line_ink_layout ? &line_ink : NULL,
                                            &line_logical,
                                            &height);
 ...
 */
static void
ns_pango_layout_get_extents_internal (NsPangoLayout    *layout,
                                   NsPangoRectangle *ink_rect,
                                   NsPangoRectangle *logical_rect,
  ...
  if (layout->auto_dir)
    {
      ...
      while (line_list && !need_width)
        {
          NsPangoLayoutLine *line = line_list->data;

          if (get_alignment (layout, line) != NS_PANGO_ALIGN_LEFT)
            need_width = TRUE;

          line_list = line_list->next;
        }
    }
  else if (layout->alignment != NS_PANGO_ALIGN_LEFT)
    need_width = TRUE;

  if (width == -1 && need_width && (ink_rect || line_extents))
    {
      NsPangoRectangle overall_logical;

      ns_pango_layout_get_extents_internal (layout, NULL, &overall_logical, NULL);
 ...
 */
void
ns_pango_layout_get_extents (NsPangoLayout    *layout,
                          NsPangoRectangle *ink_rect,
                          NsPangoRectangle *logical_rect)
{
  g_return_if_fail (layout != NULL);

  ns_pango_layout_get_extents_internal (layout, ink_rect, logical_rect, NULL);
}

/**
 * ns_pango_layout_get_pixel_extents:
 * @layout: a `NsPangoLayout`
 * @ink_rect: (out) (optional): rectangle used to store the extents of the
 *   layout as drawn
 * @logical_rect: (out) (optional): rectangle used to store the logical
 ...
 */
void
ns_pango_layout_get_pixel_extents (NsPangoLayout    *layout,
                                NsPangoRectangle *ink_rect,
                                NsPangoRectangle *logical_rect)
{
  g_return_if_fail (NS_PANGO_IS_LAYOUT (layout));

  ns_pango_layout_get_extents (layout, ink_rect, logical_rect);
  ns_pango_extents_to_pixels (ink_rect, NULL);
  ns_pango_extents_to_pixels (logical_rect, NULL);
}

/**
 * ns_pango_layout_get_size:
 * @layout: a `NsPangoLayout`
 * @width: (out) (optional): location to store the logical width
 * @height: (out) (optional): location to store the logical height
 *
 * Determines the logical width and height of a `NsPangoLayout` in Pango
 * units.
 *
 * This is simply a convenience function around [method@Pango.Layout.get_extents].
 */
void
ns_pango_layout_get_size (NsPangoLayout *layout,
                       int         *width,
                       int         *height)
{
  NsPangoRectangle logical_rect;

  ns_pango_layout_get_extents (layout, NULL, &logical_rect);
...
/**
 * ns_pango_layout_get_pixel_size:
 * @layout: a `NsPangoLayout`
 * @width: (out) (optional): location to store the logical width
 * @height: (out) (optional): location to store the logical height
 *
 * Determines the logical width and height of a `NsPangoLayout` in device
 * units.
 *
 * [method@Pango.Layout.get_size] returns the width and height
 * scaled by %NS_PANGO_SCALE. This is simply a convenience function
 * around [method@Pango.Layout.get_pixel_extents].
 */
void
ns_pango_layout_get_pixel_size (NsPangoLayout *layout,
                             int         *width,
                             int         *height)
{
  NsPangoRectangle logical_rect;

  ns_pango_layout_get_extents_internal (layout, NULL, &logical_rect, NULL);
  ns_pango_extents_to_pixels (&logical_rect, NULL);
 ...
 */
int
ns_pango_layout_get_baseline (NsPangoLayout *layout)
{
  int baseline;
  Extents *extents = NULL;

  /* XXX this is kinda inefficient */
  ns_pango_layout_get_extents_internal (layout, NULL, NULL, &extents);
...
static void
ns_pango_layout_clear_lines (NsPangoLayout *layout)
{
  if (layout->lines)
    {
      GSList *tmp_list = layout->lines;
      while (tmp_list)
        {
          ...
          line->layout = NULL;
          ns_pango_layout_line_unref (line);
        }
...
static void
ns_pango_layout_line_leaked (NsPangoLayoutLine *line)
{
  NsPangoLayoutLinePrivate *private = (NsPangoLayoutLinePrivate *)line;
...
static void shape_tab (NsPangoLayoutLine  *line,
...
static void
free_run (NsPangoLayoutRun *run, gpointer data)
{
  gboolean free_item = data != NULL;
  if (free_item)
    ns_pango_item_free (run->item);

  ns_pango_glyph_string_free (run->glyphs);
  g_slice_free (NsPangoLayoutRun, run);
}

static NsPangoItem *
uninsert_run (NsPangoLayoutLine *line)
{
  NsPangoLayoutRun *run;
  NsPangoItem *item;
...
static void
ensure_tab_width (NsPangoLayout *layout)
{
  if (layout->tab_width == -1)
    {
      ...
      NsPangoAttrList *layout_attrs;
      NsPangoAttrList tmp_attrs;
      NsPangoFontDescription *font_desc = ns_pango_font_description_copy_static (ns_pango_context_get_font_description (layout->context));
      NsPangoLanguage *language = NULL;
      NsPangoShapeFlags shape_flags = NS_PANGO_SHAPE_NONE;
      const char *eight_spaces = "        ";

      if (ns_pango_context_get_round_glyph_positions (layout->context))
        shape_flags |= NS_PANGO_SHAPE_ROUND_POSITIONS;

      layout_attrs = ns_pango_layout_get_effective_attributes (layout);
      if (layout_attrs)
        {
          NsPangoAttrIterator iter;

          _ns_pango_attr_list_get_iterator (layout_attrs, &iter);
          ns_pango_attr_iterator_get_font (&iter, font_desc, &language, NULL);
          _ns_pango_attr_iterator_destroy (&iter);
        }

      _ns_pango_attr_list_init (&tmp_attrs);

      attr = ns_pango_attr_font_desc_new (font_desc);
      ns_pango_font_description_free (font_desc);
      ns_pango_attr_list_insert_before (&tmp_attrs, attr);

      if (language)
        {
          attr = ns_pango_attr_language_new (language);
          ns_pango_attr_list_insert_before (&tmp_attrs, attr);
        }

      items = ns_pango_itemize (layout->context, " ", 0, 1, &tmp_attrs, NULL);

      if (layout_attrs != layout->attrs)
        {
          ns_pango_attr_list_unref (layout_attrs);
          layout_attrs = NULL;
        }
      _ns_pango_attr_list_destroy (&tmp_attrs);

      item = items->data;
      ns_pango_shape_with_flags (eight_spaces, 8, eight_spaces, 8, &item->analysis, glyphs, shape_flags);

      ns_pango_item_free (item);
      g_list_free (items);

      layout->tab_width = ns_pango_glyph_string_get_width (glyphs);
      ...
      if (layout->tab_width <= 0)
        layout->tab_width = 50 * NS_PANGO_SCALE; /* pretty much arbitrary */
    }
}

static void
get_tab_pos (NsPangoLayoutLine *line,
             int              index,
             int             *tab_pos,
             ...
             gboolean        *is_default)
{
  ...
  if (layout->alignment != NS_PANGO_ALIGN_CENTER)
    {
  ...
  if (layout->tabs)
    {
      n_tabs = ns_pango_tab_array_get_size (layout->tabs);
      in_pixels = ns_pango_tab_array_get_positions_in_pixels (layout->tabs);
      *is_default = FALSE;
    }
  else
    {
  ...
  if (index < n_tabs)
    {
      ns_pango_tab_array_get_tab (layout->tabs, index, alignment, tab_pos);

      if (in_pixels)
        *tab_pos *= NS_PANGO_SCALE;

      *decimal = ns_pango_tab_array_get_decimal_point (layout->tabs, index);
    }
  else if (n_tabs > 0)
    {
      ...
      ns_pango_tab_array_get_tab (layout->tabs, n_tabs - 1, alignment, &last_pos);
      *decimal = ns_pango_tab_array_get_decimal_point (layout->tabs, n_tabs - 1);

      if (n_tabs > 1)
        ns_pango_tab_array_get_tab (layout->tabs, n_tabs - 2, NULL, &next_to_last_pos);
      else
        next_to_last_pos = 0;

      if (in_pixels)
        {
          next_to_last_pos *= NS_PANGO_SCALE;
          last_pos *= NS_PANGO_SCALE;
        }
...
static void
ensure_decimal (NsPangoLayout *layout)
{
  if (layout->decimal == 0)
    {
#ifndef __BIONIC__
...
static void
shape_tab (NsPangoLayoutLine  *line,
           LastTabState     *tab_state,
           ItemProperties   *properties,
           int               current_width,
           NsPangoItem        *item,
           NsPangoGlyphString *glyphs)
{
  int i, space_width;
  int tab_pos;
  NsPangoTabAlign tab_align;
  gunichar tab_decimal;

  ns_pango_glyph_string_set_size (glyphs, 1);

  if (properties->showing_space)
    glyphs->glyphs[0].glyph = NS_PANGO_GET_UNKNOWN_GLYPH ('\t');
  else
    glyphs->glyphs[0].glyph = NS_PANGO_GLYPH_EMPTY;
...
can_break_at (NsPangoLayout   *layout,
              gint           offset,
              NsPangoWrapMode  wrap)
{
  if (offset == layout->n_chars)
    return TRUE;
  else if (wrap == NS_PANGO_WRAP_NONE)
    return FALSE;
  else if (wrap == NS_PANGO_WRAP_CHAR)
...
static inline gboolean
can_break_in (NsPangoLayout *layout,
              int          start_offset,
              int          num_chars,
                           ...
                           int *space_right)
{
  *space_left = letter_spacing / 2;
  /* hinting */
  if ((letter_spacing & (NS_PANGO_SCALE - 1)) == 0)
    {
      *space_left = NS_PANGO_UNITS_ROUND (*space_left);
    }
...
struct _ParaBreakState
{
  ...
  NsPangoAttrList *attrs;         /* Attributes being used for itemization */
  GList *items;                 /* This paragraph turned into items */
  NsPangoDirection base_dir;      /* Current resolved base direction */
  int line_of_par;              /* Line of the paragraph, starting at 1 for first line */

  NsPangoGlyphString *glyphs;     /* Glyphs for the first item in state->items */
...
static gboolean
should_ellipsize_current_line (NsPangoLayout    *layout,
                               ParaBreakState *state);

static void
get_decimal_prefix_width (NsPangoItem        *item,
                          NsPangoGlyphString *glyphs,
                          const char       *text,
                          gunichar          decimal,
                          int              *width,
                          gboolean         *found)
{
  NsPangoGlyphItem glyph_item = { item, glyphs, 0, 0, 0 };
  int *log_widths;
  int i;
  const char *p;
  ...
  ns_pango_glyph_item_get_logical_widths (&glyph_item, text, log_widths);
...
static NsPangoGlyphString *
shape_run (NsPangoLayoutLine *line,
           ParaBreakState  *state,
           NsPangoItem       *item)
{
  NsPangoLayout *layout = line->layout;
  ...
  if (layout->text[item->offset] == '\t')
  ...
  else
    {
      ...
      if (state->properties.shape_set)
        _ns_pango_shape_shape (layout->text + item->offset, item->num_chars,
                            state->properties.shape_ink_rect, state->properties.shape_logical_rect,
                            glyphs);
      else
        ns_pango_shape_item (item,
      ...
      if (state->properties.letter_spacing)
        {
          ...
          ns_pango_glyph_item_letter_space (&glyph_item,
                                         layout->text,
                                         layout->log_attrs + state->start_offset,
      ...
      if (state->last_tab.glyphs != NULL)
        {
          ...
          if (state->last_tab.align == NS_PANGO_TAB_RIGHT)
            w -= ns_pango_glyph_string_get_width (glyphs);
          else if (state->last_tab.align == NS_PANGO_TAB_CENTER)
            w -= ns_pango_glyph_string_get_width (glyphs) / 2;
          else if (state->last_tab.align == NS_PANGO_TAB_DECIMAL)
            {
              int width;
              gboolean found;
...
static void
insert_run (NsPangoLayoutLine  *line,
            ParaBreakState   *state,
            NsPangoItem        *run_item,
            NsPangoGlyphString *glyphs,
            gboolean          last_run)
{
  NsPangoLayoutRun *run = g_slice_new (NsPangoLayoutRun);
  ...
  if (glyphs)
    run->glyphs = glyphs;
           ...
           state->glyphs &&
           !(run_item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_NEED_HYPHEN))
    {
  ...
  if (last_run && state->glyphs)
    {
      ns_pango_glyph_string_free (state->glyphs);
      state->glyphs = NULL;
    }
  ...
  if (state->last_tab.glyphs && run->glyphs != state->last_tab.glyphs)
    {
      ...
      if (state->last_tab.align == NS_PANGO_TAB_RIGHT)
        state->last_tab.width += ns_pango_glyph_string_get_width (run->glyphs);
      else if (state->last_tab.align == NS_PANGO_TAB_CENTER)
        state->last_tab.width += ns_pango_glyph_string_get_width (run->glyphs) / 2;
      else if (state->last_tab.align == NS_PANGO_TAB_DECIMAL)
        {
          int width;
...
static gboolean
break_needs_hyphen (NsPangoLayout    *layout,
                    ParaBreakState *state,
                    int             pos)
{
...
static int
find_hyphen_width (NsPangoItem *item)
{
  hb_font_t *hb_font;
...
ensure_hyphen_width (ParaBreakState *state)
{
  if (state->hyphen_width < 0)
    {
      NsPangoItem *item = state->items->data;
      state->hyphen_width = find_hyphen_width (item);
    }
}

static int
find_break_extra_width (NsPangoLayout    *layout,
                        ParaBreakState *state,
                        int             pos)
{
...
#if 0
# define DEBUG debug
static int ns_pango_layout_line_get_width (NsPangoLayoutLine *line);
static void
debug (const char *where, NsPangoLayoutLine *line, ParaBreakState *state)
{
  int line_width = ns_pango_layout_line_get_width (line);
...
static inline void
compute_log_widths (NsPangoLayout    *layout,
                    ParaBreakState *state)
{
  NsPangoItem *item = state->items->data;
  NsPangoGlyphItem glyph_item = { item, state->glyphs };
  ...
  ns_pango_glyph_item_get_logical_widths (&glyph_item, layout->text, state->log_widths);
}
 ...
 */
static BreakResult
process_item (NsPangoLayout     *layout,
              NsPangoLayoutLine *line,
              ParaBreakState  *state,
              gboolean         force_fit,
              ...
              gboolean         is_last_item)
{
  ...
  NsPangoGlyphString *break_glyphs;
  ...
  if (!state->glyphs)
    {
      ns_pango_layout_get_item_properties (item, &state->properties);
...
#define LINE_SEPARATOR 0x2028
  ...
  if (processing_new_item)
    {
      width = ns_pango_glyph_string_get_width (state->glyphs);
    }
  else
    {
      width = 0;
  ...
  if ((width + extra_width <= state->remaining_width || (item->num_chars == 1 && !line->runs) ||
      (state->last_tab.glyphs && state->last_tab.align != NS_PANGO_TAB_LEFT)) &&
      !no_break_at_end)
    {
      NsPangoGlyphString *glyphs;
      ...
      width = ns_pango_glyph_string_get_width (glyphs) + tab_width_change (state);
   ...
   */
  metrics = ns_pango_font_get_metrics (item->analysis.font, item->analysis.language);
  safe_distance = ns_pango_font_metrics_get_approximate_char_width (metrics) * 3;
...
retry_break:
  ...
  for (num_chars = 0, width = 0; num_chars < (no_break_at_end ? item->num_chars : (item->num_chars + 1)); num_chars++)
          ...
          if (num_chars == 0 ||
          ...
          else
            {
              ...
              if (num_chars < item->num_chars)
                {
                  new_item = ns_pango_item_split (item, length, num_chars);

                  if (break_needs_hyphen (layout, state, num_chars))
                    new_item->analysis.flags |= NS_PANGO_ANALYSIS_FLAG_NEED_HYPHEN;
                  else
                    new_item->analysis.flags &= ~NS_PANGO_ANALYSIS_FLAG_NEED_HYPHEN;
                }
              else
              ...
              new_break_width = ns_pango_glyph_string_get_width (glyphs) + tab_width_change (state);
              ...
              if (new_item != item)
                {
                  ns_pango_item_free (new_item);
                  ns_pango_item_unsplit (item, length, num_chars);
                }
              ...
              if (break_num_chars == item->num_chars ||
                  ...
                  new_break_width + extra_width < break_width + break_extra_width)
                {
                  ...
                  if (break_glyphs)
                    ns_pango_glyph_string_free (break_glyphs);
                  break_glyphs = glyphs;
                }
              else
                {
   ...
   if (wrap == NS_PANGO_WRAP_WORD_CHAR && force_fit && break_width + break_extra_width > state->remaining_width)
    {
      /* Try again, with looser conditions */
      DEBUG1 ("does not fit, try again with wrap-char");
      wrap = NS_PANGO_WRAP_CHAR;
      break_num_chars = item->num_chars;
      break_width = orig_width;
      break_extra_width = orig_extra_width;
      g_clear_pointer (&break_glyphs, ns_pango_glyph_string_free);
      goto retry_break;
    }
      ...
      if (break_num_chars == item->num_chars)
        {
          if (can_break_at (layout, state->start_offset + break_num_chars, wrap) &&
            ...
            item->analysis.flags |= NS_PANGO_ANALYSIS_FLAG_NEED_HYPHEN;
      ...
      else
        {
          ...
          new_item = ns_pango_item_split (item, length, break_num_chars);
 ...
 */
static void
line_set_resolved_dir (NsPangoLayoutLine *line,
                       NsPangoDirection   direction)
{
  switch (direction)
    ...
    case NS_PANGO_DIRECTION_WEAK_LTR:
    case NS_PANGO_DIRECTION_NEUTRAL:
      line->resolved_dir = NS_PANGO_DIRECTION_LTR;
      break;
    case NS_PANGO_DIRECTION_RTL:
    case NS_PANGO_DIRECTION_WEAK_RTL:
    case NS_PANGO_DIRECTION_TTB_LTR:
      line->resolved_dir = NS_PANGO_DIRECTION_RTL;
      break;
    }
   ...
   */
  switch (ns_pango_context_get_gravity (line->layout->context))
    {
    default:
...
should_ellipsize_current_line (NsPangoLayout    *layout,
                               ParaBreakState *state)
{
  if (G_LIKELY (layout->ellipsize == NS_PANGO_ELLIPSIZE_NONE || layout->width < 0))
    return FALSE;

  if (layout->wrap == NS_PANGO_WRAP_NONE)
    return TRUE;
...
static void
add_line (NsPangoLayoutLine *line,
          ParaBreakState  *state)
{
  NsPangoLayout *layout = line->layout;
  ...
  if (layout->height >= 0)
    {
      NsPangoRectangle logical_rect;
      ns_pango_layout_line_get_extents (line, NULL, &logical_rect);
...
static void
process_line (NsPangoLayout    *layout,
              ParaBreakState *state)
{
  NsPangoLayoutLine *line;
  ...
  line = ns_pango_layout_line_new (layout);
  ...
  if (state->line_width >= 0 && layout->alignment != NS_PANGO_ALIGN_CENTER)
    {
      if (line->is_paragraph_start && layout->indent >= 0)
  ...
  state->last_tab.align = NS_PANGO_TAB_LEFT;
      ...
      switch (result)
        ...
        case BREAK_NONE_FIT:
          ...
          while (line->runs && line->runs != break_link)
            {
              NsPangoLayoutRun *run = line->runs->data;
  ...
  ns_pango_layout_line_postprocess (line, state, wrapped);
                     ...
                     int            log_attrs_len)
{
  ...
  ns_pango_default_break (text + start, length, NULL, log_attrs, log_attrs_len);
  ...
  for (l = items; l; l = l->next)
    {
      ...
      ns_pango_tailor_break (text + item->offset,
                          item->length,
                          &item->analysis,
                          -1,
                          log_attrs + offset,
  ...
  if (attrs && items)
    {
      NsPangoItem *item = items->data;
      ns_pango_attr_break (text + start, length, attrs, item->offset, log_attrs, log_attrs_len);
    }
}

static NsPangoAttrList *
ns_pango_layout_get_effective_attributes (NsPangoLayout *layout)
{
  NsPangoAttrList *attrs;

  if (layout->attrs)
    attrs = ns_pango_attr_list_copy (layout->attrs);
  else
    attrs = NULL;

  if (layout->font_desc)
    {
      NsPangoAttribute *attr = ns_pango_attr_font_desc_new (layout->font_desc);

      if (!attrs)
        attrs = ns_pango_attr_list_new ();

      ns_pango_attr_list_insert_before (attrs, attr);
    }

  if (layout->single_paragraph)
    {
      NsPangoAttribute *attr = ns_pango_attr_show_new (NS_PANGO_SHOW_LINE_BREAKS);
...
static gboolean
affects_itemization (NsPangoAttribute *attr,
                     gpointer        data)
{
...
static gboolean
affects_break_or_shape (NsPangoAttribute *attr,
                        gpointer        data)
{
  switch ((int)attr->klass->type)
    {
                           ...
                           NsPangoAttrList *attrs)
{
  ...
  _ns_pango_attr_list_get_iterator (attrs, &iter);

  for (l = items; l; l = l->next)
    {
      NsPangoItem *item = l->data;
      ns_pango_item_apply_attrs (item, &iter);
    }
...
static void
apply_attributes_to_runs (NsPangoLayout   *layout,
                          NsPangoAttrList *attrs)
{
  GSList *ll;
  ...
  for (ll = layout->lines; ll; ll = ll->next)
    {
      NsPangoLayoutLine *line = ll->data;
      ...
      for (rl = old_runs; rl; rl = rl->next)
        {
          NsPangoGlyphItem *glyph_item = rl->data;
          GSList *new_runs;

          new_runs = ns_pango_glyph_item_apply_attrs (glyph_item,
                                                   layout->text,
                                                   attrs);
...
static void
ns_pango_layout_check_lines (NsPangoLayout *layout)
{
  const char *start;
  gboolean done = FALSE;
  int start_offset;
  NsPangoAttrList *attrs;
  NsPangoAttrList *itemize_attrs;
  NsPangoAttrList *shape_attrs;
  NsPangoAttrIterator iter;
  NsPangoDirection prev_base_dir = NS_PANGO_DIRECTION_NEUTRAL;
  NsPangoDirection base_dir = NS_PANGO_DIRECTION_NEUTRAL;
  ParaBreakState state;
   ...
   */
  if (G_UNLIKELY (!layout->text))
    ns_pango_layout_set_text (layout, NULL, 0);

  attrs = ns_pango_layout_get_effective_attributes (layout);
  if (attrs)
    {
      shape_attrs = ns_pango_attr_list_filter (attrs, affects_break_or_shape, NULL);
      itemize_attrs = ns_pango_attr_list_filter (attrs, affects_itemization, NULL);

      if (itemize_attrs)
        _ns_pango_attr_list_get_iterator (itemize_attrs, &iter);
    }
  else
    {
      shape_attrs = NULL;
  ...
  if (!layout->log_attrs)
    {
      layout->log_attrs = g_new0 (NsPangoLogAttr, layout->n_chars + 1);
      need_log_attrs = TRUE;
    }
  else
    {
  ...
  if (layout->auto_dir)
    {
      prev_base_dir = ns_pango_find_base_dir (layout->text, layout->length);
      if (prev_base_dir == NS_PANGO_DIRECTION_NEUTRAL)
        prev_base_dir = ns_pango_context_get_base_dir (layout->context);
    }
  else
    base_dir = ns_pango_context_get_base_dir (layout->context);
  ...
  if (layout->height >= 0)
    {
      NsPangoRectangle logical = { 0, };
      int height = 0;
      ns_pango_layout_get_empty_extents_and_height_at_index (layout, 0, &logical, TRUE, &height);
  ...
  do
    {
      ...
      if (layout->single_paragraph)
        ...
        {
          ns_pango_find_paragraph_boundary (start,
      ...
      if (layout->auto_dir)
        {
          base_dir = ns_pango_find_base_dir (start, delimiter_index);

          /* Propagate the base direction for neutral paragraphs */
          if (base_dir == NS_PANGO_DIRECTION_NEUTRAL)
      ...
      state.items = ns_pango_itemize_with_font (layout->context,
                                             base_dir,
                                             layout->text,
                                             start - layout->text,
                                             end - start,
      ...
      state.items = ns_pango_itemize_post_process_items (layout->context,
                                                      layout->text,
                                                      layout->log_attrs,
                                                      state.items);
      ...
      if (state.items)
        ...
        {
          NsPangoLayoutLine *empty_line;

          empty_line = ns_pango_layout_line_new (layout);
      ...
      if (!done)
        start_offset += ns_pango_utf8_strlen (start, (end - start) + delim_len);
  ...
  if (itemize_attrs)
    {
      ns_pango_attr_list_unref (itemize_attrs);
      _ns_pango_attr_iterator_destroy (&iter);
    }

  ns_pango_attr_list_unref (shape_attrs);
...
/**
 * ns_pango_layout_line_ref:
 * @line: (nullable): a `NsPangoLayoutLine`
 *
 * Increase the reference count of a `NsPangoLayoutLine` by one.
 *
 ...
 */
NsPangoLayoutLine *
ns_pango_layout_line_ref (NsPangoLayoutLine *line)
{
  NsPangoLayoutLinePrivate *private = (NsPangoLayoutLinePrivate *)line;
...
/**
 * ns_pango_layout_line_unref:
 * @line: a `NsPangoLayoutLine`
 *
 * Decrease the reference count of a `NsPangoLayoutLine` by one.
 *
 * If the result is zero, the line and all associated memory
 * will be freed.
 */
void
ns_pango_layout_line_unref (NsPangoLayoutLine *line)
{
  NsPangoLayoutLinePrivate *private = (NsPangoLayoutLinePrivate *)line;
  ...
  if (g_atomic_int_dec_and_test ((int *) &private->ref_count))
    {
      ...
      g_slice_free (NsPangoLayoutLinePrivate, private);
    }
}

G_DEFINE_BOXED_TYPE (NsPangoLayoutLine, ns_pango_layout_line,
                     ns_pango_layout_line_ref,
                     ns_pango_layout_line_unref);

/**
 * ns_pango_layout_line_get_start_index:
 * @line: a `NsPangoLayoutLine`
 *
 ...
 */
int
ns_pango_layout_line_get_start_index (NsPangoLayoutLine *line)
{
  return line->start_index;
}

/**
 * ns_pango_layout_line_get_length:
 * @line: a `NsPangoLayoutLine`
 *
 ...
 */
int
ns_pango_layout_line_get_length (NsPangoLayoutLine *line)
{
  return line->length;
}

/**
 * ns_pango_layout_line_is_paragraph_start:
 * @line: a `NsPangoLayoutLine`
 *
 ...
 */
gboolean
ns_pango_layout_line_is_paragraph_start (NsPangoLayoutLine *line)
{
  return line->is_paragraph_start;
}

/**
 * ns_pango_layout_line_get_resolved_direction:
 * @line: a `NsPangoLayoutLine`
 *
 ...
 */
NsPangoDirection
ns_pango_layout_line_get_resolved_direction (NsPangoLayoutLine *line)
{
  return (NsPangoDirection) line->resolved_dir;
}

/**
 * ns_pango_layout_line_x_to_index:
 * @line: a `NsPangoLayoutLine`
 ...
 */
gboolean
ns_pango_layout_line_x_to_index (NsPangoLayoutLine *line,
                              int              x_pos,
                              int             *index,
                              int             *trailing)
{
  ...
  if (tmp_list->next &&
      line->start_index + line->length == ((NsPangoLayoutLine *)tmp_list->next->data)->start_index)
  ...
  if (x_pos < 0)
    {
      ...
      if (index)
        *index = (line->resolved_dir == NS_PANGO_DIRECTION_LTR) ? first_index : last_index;
      /* and its leftmost edge */
      if (trailing)
        *trailing = (line->resolved_dir == NS_PANGO_DIRECTION_LTR || suppress_last_trailing) ? 0 : last_trailing;
  ...
  while (tmp_list)
    {
      NsPangoLayoutRun *run = tmp_list->data;
      int logical_width;

      logical_width = ns_pango_glyph_string_get_width (run->glyphs);
      ...
      if (x_pos >= start_pos && x_pos < start_pos + logical_width)
        {
          ...
          ns_pango_glyph_string_x_to_index (run->glyphs,
  ...
  if (index)
    *index = (line->resolved_dir == NS_PANGO_DIRECTION_LTR) ? last_index : first_index;

  /* and its rightmost edge */
  if (trailing)
    *trailing = (line->resolved_dir == NS_PANGO_DIRECTION_LTR && !suppress_last_trailing) ? last_trailing : 0;

  return FALSE;
}

static int
ns_pango_layout_line_get_width (NsPangoLayoutLine *line)
{
  int width = 0;
...
/**
 * ns_pango_layout_line_get_x_ranges:
 * @line: a `NsPangoLayoutLine`
 ...
 */
void
ns_pango_layout_line_get_x_ranges (NsPangoLayoutLine  *line,
                                int               start_index,
                                int               end_index,
                                int             **ranges,
                                int              *n_ranges)
{
  ...
  if (width == -1 && alignment != NS_PANGO_ALIGN_LEFT)
    {
      NsPangoRectangle logical_rect;
      ns_pango_layout_get_extents (line->layout, NULL, &logical_rect);
      width = logical_rect.width;
    }
  ...
  if (x_offset > 0 &&
      ((line->resolved_dir == NS_PANGO_DIRECTION_LTR && start_index < line_start_index) ||
       (line->resolved_dir == NS_PANGO_DIRECTION_RTL && end_index > line_start_index + line->length)))
    {
      if (ranges)
        {
  ...
  while (tmp_list)
    {
      NsPangoLayoutRun *run = (NsPangoLayoutRun *)tmp_list->data;
      ...
      if (tmp_list->next)
        accumulated_width += ns_pango_glyph_string_get_width (run->glyphs);

      tmp_list = tmp_list->next;
    }

  if (x_offset + line_width < line->layout->width &&
      ((line->resolved_dir == NS_PANGO_DIRECTION_LTR && end_index > line_start_index + line->length) ||
       (line->resolved_dir == NS_PANGO_DIRECTION_RTL && start_index < line_start_index)))
    {
      if (ranges)
        {
...
static void
ns_pango_layout_get_empty_extents_and_height_at_index (NsPangoLayout    *layout,
                                                    int             index,
                                                    NsPangoRectangle *logical_rect,
                                                    gboolean        apply_line_height,
                                                    int             *height)
{
  if (logical_rect)
    {
      NsPangoFont *font;
      NsPangoFontDescription *font_desc = NULL;
      gboolean free_font_desc = FALSE;
      double line_height_factor = 0.0;
      int absolute_line_height = 0;

      font_desc = ns_pango_context_get_font_description (layout->context);

      if (layout->font_desc)
        {
          font_desc = ns_pango_font_description_copy_static (font_desc);
          ns_pango_font_description_merge (font_desc, layout->font_desc, TRUE);
          free_font_desc = TRUE;
        }
      ...
      if (layout->attrs)
        {
          ...
          _ns_pango_attr_list_get_iterator (layout->attrs, &iter);
              ...
              if (start <= index && index < end)
                {
                  ...
                  ns_pango_attr_iterator_get_font (&iter, font_desc, NULL, NULL);

                  attr = ns_pango_attr_iterator_get (&iter, NS_PANGO_ATTR_LINE_HEIGHT);
                  if (attr)
                    line_height_factor = ((NsPangoAttrFloat *)attr)->value;

                  attr = ns_pango_attr_iterator_get (&iter, NS_PANGO_ATTR_ABSOLUTE_LINE_HEIGHT);
                  if (attr)
                    absolute_line_height = ((NsPangoAttrInt *)attr)->value;
      ...
      font = ns_pango_context_load_font (layout->context, font_desc);
      if (font)
        {
          NsPangoFontMetrics *metrics;

          metrics = ns_pango_font_get_metrics (font,
                                            ns_pango_context_get_language (layout->context));

          if (metrics)
            {
              logical_rect->y = - ns_pango_font_metrics_get_ascent (metrics);
              logical_rect->height = - logical_rect->y + ns_pango_font_metrics_get_descent (metrics);
              if (height)
                *height = ns_pango_font_metrics_get_height (metrics);
...
static void
ns_pango_layout_run_get_extents_and_height (NsPangoLayoutRun *run,
                                         NsPangoRectangle *run_ink,
                                         NsPangoRectangle *run_logical,
                                         NsPangoRectangle *line_logical,
                                         int            *height)
{
  NsPangoRectangle logical;
  ItemProperties properties;
  NsPangoFontMetrics *metrics = NULL;
  ...
  ns_pango_layout_get_item_properties (run->item, &properties);
  ...
  if (!run_logical && (run->item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_CENTERED_BASELINE))
    run_logical = &logical;
  ...
  if (properties.shape_set && !(run->item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_IS_ELLIPSIS))
    _ns_pango_shape_get_extents (run->item->num_chars,
  ...
  if (run_ink && (has_underline || has_overline || properties.strikethrough))
    {
      ...
      underline_thickness = ns_pango_font_metrics_get_underline_thickness (metrics);
      underline_position = ns_pango_font_metrics_get_underline_position (metrics);
      strikethrough_thickness = ns_pango_font_metrics_get_strikethrough_thickness (metrics);
      strikethrough_position = ns_pango_font_metrics_get_strikethrough_position (metrics);
  ...
  if (height)
    {
      if (ns_pango_analysis_get_size_font (&run->item->analysis))
        {
          NsPangoFontMetrics *height_metrics;
          double xscale, yscale;

          height_metrics = ns_pango_font_get_metrics (ns_pango_analysis_get_size_font (&run->item->analysis),
                                                   run->item->analysis.language);

          ns_pango_font_get_scale_factors (ns_pango_analysis_get_size_font (&run->item->analysis), &xscale, &yscale);
          *height = ns_pango_font_metrics_get_height (height_metrics) * MAX (xscale, yscale);

          ns_pango_font_metrics_unref (height_metrics);
        }
      else
        {
          double xscale, yscale;
          ...
          ns_pango_font_get_scale_factors (run->item->analysis.font, &xscale, &yscale);
          *height = ns_pango_font_metrics_get_height (metrics) * MAX (xscale, yscale);
        }
    }
  ...
  if (run->item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_CENTERED_BASELINE)
    {
      gboolean is_hinted = (run_logical->y & run_logical->height & (NS_PANGO_SCALE - 1)) == 0;
                                          ...
                                          int             *height)
{
  ...
  while (tmp_list)
    {
      ...
      ns_pango_layout_run_get_extents_and_height (run,
  ...
  if (!line->runs)
    {
      NsPangoRectangle r, *rect;

      rect = logical_rect ? logical_rect : &r;
      ns_pango_layout_get_empty_extents_and_height_at_index (line->layout, line->start_index, rect, TRUE, height);
    }
...
/**
 * ns_pango_layout_line_get_extents:
 * @line: a `NsPangoLayoutLine`
 ...
 */
void
ns_pango_layout_line_get_extents (NsPangoLayoutLine *line,
                               NsPangoRectangle  *ink_rect,
                               NsPangoRectangle  *logical_rect)
{
  ns_pango_layout_line_get_extents_and_height (line, ink_rect, logical_rect, NULL);
}

/**
 * ns_pango_layout_line_get_height:
 * @line: a `NsPangoLayoutLine`
 ...
 */
void
ns_pango_layout_line_get_height (NsPangoLayoutLine *line,
                              int             *height)
{
  ns_pango_layout_line_get_extents_and_height (line, NULL, NULL, height);
}

static NsPangoLayoutLine *
ns_pango_layout_line_new (NsPangoLayout *layout)
{
  NsPangoLayoutLinePrivate *private = g_slice_new (NsPangoLayoutLinePrivate);
  ...
  return (NsPangoLayoutLine *) private;
}

/**
 * ns_pango_layout_line_get_pixel_extents:
 * @layout_line: a `NsPangoLayoutLine`
 * @ink_rect: (out) (optional): rectangle used to store the extents of
 *   the glyph string as drawn
 * @logical_rect: (out) (optional): rectangle used to store the logical
 *   extents of the glyph string
 ...
 */
void
ns_pango_layout_line_get_pixel_extents (NsPangoLayoutLine *layout_line,
                                     NsPangoRectangle  *ink_rect,
                                     NsPangoRectangle  *logical_rect)
{
  g_return_if_fail (LINE_IS_VALID (layout_line));

  ns_pango_layout_line_get_extents (layout_line, ink_rect, logical_rect);
  ns_pango_extents_to_pixels (ink_rect, NULL);
  ns_pango_extents_to_pixels (logical_rect, NULL);
}
 ...
 *     reorder-items.c:ns_pango_reorder_items().
 */
static GSList *
...
static void
ns_pango_layout_line_reorder (NsPangoLayoutLine *line)
{
...
static int
get_item_letter_spacing (NsPangoItem *item)
{
  ItemProperties properties;

  ns_pango_layout_get_item_properties (item, &properties);

  return properties.letter_spacing;
}

static void
pad_glyphstring_right (NsPangoGlyphString *glyphs,
                       ParaBreakState   *state,
                       int               adjustment)
{
...
static void
pad_glyphstring_left (NsPangoGlyphString *glyphs,
                      ParaBreakState   *state,
                      int               adjustment)
{
  int glyph = 0;
...
static gboolean
is_tab_run (NsPangoLayout    *layout,
            NsPangoLayoutRun *run)
{
...
static void
add_missing_hyphen (NsPangoLayoutLine *line,
                    ParaBreakState  *state,
                    NsPangoLayoutRun  *run)
{
  NsPangoLayout *layout = line->layout;
  NsPangoItem *item = run->item;
  int line_chars;

  line_chars = 0;
  for (GSList *l = line->runs; l; l = l->next)
    {
      NsPangoLayoutRun *r = l->data;
  ...
  if (layout->log_attrs[state->line_start_offset + line_chars].break_inserts_hyphen &&
      !(item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_NEED_HYPHEN))
    {
      ...
      state->remaining_width += ns_pango_glyph_string_get_width (run->glyphs) - width;
    }
}

static NsPangoShowFlags
find_show_flags (const NsPangoAnalysis *analysis)
{
  GSList *l;
  NsPangoShowFlags flags = 0;

  for (l = analysis->extra_attrs; l; l = l->next)
    {
      NsPangoAttribute *attr = l->data;

      if (attr->klass->type == NS_PANGO_ATTR_SHOW)
        flags |= ((NsPangoAttrInt*)attr)->value;
    }
...
static void
zero_line_final_space (NsPangoLayoutLine *line,
                       ParaBreakState  *state,
                       NsPangoLayoutRun  *run)
{
  NsPangoLayout *layout = line->layout;
  ...
  if (glyphs->glyphs[glyph].glyph == NS_PANGO_GET_UNKNOWN_GLYPH (0x2028))
    {
      NsPangoShowFlags show_flags;

      show_flags = find_show_flags (&item->analysis);

      if ((show_flags & NS_PANGO_SHOW_LINE_BREAKS) != 0)
        {
  ...
  glyphs->glyphs[glyph].geometry.width = 0;
  glyphs->glyphs[glyph].glyph = NS_PANGO_GLYPH_EMPTY;
}
 ...
 */
static void
adjust_line_letter_spacing (NsPangoLayoutLine *line,
                            ParaBreakState  *state)
{
  NsPangoLayout *layout = line->layout;
  gboolean reversed;
  NsPangoLayoutRun *last_run;
  int tab_adjustment;
  GSList *l;
   ...
   */
  reversed = FALSE;
  if (line->resolved_dir == NS_PANGO_DIRECTION_RTL)
    {
      for (l = line->runs; l; l = l->next)
  ...
  for (l = line->runs; l; l = l->next)
    {
      NsPangoLayoutRun *run = l->data;
      NsPangoLayoutRun *next_run = l->next ? l->next->data : NULL;
      ...
      if (is_tab_run (layout, run))
        ...
        {
          NsPangoLayoutRun *visual_next_run = reversed ? last_run : next_run;
          NsPangoLayoutRun *visual_last_run = reversed ? next_run : last_run;
...
static void
justify_clusters (NsPangoLayoutLine *line,
                  ParaBreakState  *state)
{
  const gchar *text = line->layout->text;
  const NsPangoLogAttr *log_attrs = line->layout->log_attrs;
  ...
  is_hinted = (total_remaining_width & (NS_PANGO_SCALE - 1)) == 0;

  for (mode = MEASURE; mode <= ADJUST; mode++)
    {
      gboolean leftedge = TRUE;
      NsPangoGlyphString *rightmost_glyphs = NULL;
      ...
      for (run_iter = line->runs; run_iter; run_iter = run_iter->next)
        {
          NsPangoLayoutRun *run = run_iter->data;
          NsPangoGlyphString *glyphs = run->glyphs;
           ...
           */
          g_assert (run->item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_HAS_CHAR_OFFSET);
          offset = ((NsPangoItemPrivate *)run->item)->char_offset;

          for (have_cluster = dir > 0 ?
                 ns_pango_glyph_item_iter_init_start (&cluster_iter, run, text) :
                 ns_pango_glyph_item_iter_init_end   (&cluster_iter, run, text);
               have_cluster;
               have_cluster = dir > 0 ?
                 ns_pango_glyph_item_iter_next_cluster (&cluster_iter) :
                 ns_pango_glyph_item_iter_prev_cluster (&cluster_iter))
            {
              int i;
              int width = 0;
...
static void
justify_words (NsPangoLayoutLine *line,
               ParaBreakState  *state)
{
  const gchar *text = line->layout->text;
  const NsPangoLogAttr *log_attrs = line->layout->log_attrs;
      ...
      for (run_iter = line->runs; run_iter; run_iter = run_iter->next)
        {
          ...
          for (have_cluster = ns_pango_glyph_item_iter_init_start (&cluster_iter, run, text);
               have_cluster;
  ...
  for (GSList *l = item->analysis.extra_attrs; l; l = l->next)
    {
      ...
      if (attr->klass->type == NS_PANGO_ATTR_RISE)
        {
          int value = ((NsPangoAttrInt *)attr)->value;

          *start_y_offset += value;
          *end_y_offset -= value;
        }
      else if (attr->klass->type == NS_PANGO_ATTR_BASELINE_SHIFT)
        {
          if (attr->start_index == item->offset)
            {
              ...
              value = ((NsPangoAttrInt *)attr)->value;
              ...
              if (value > 1024 || value < -1024)
              ...
              else
                {
                  ...
                  if (prev && prev->analysis.font)
                    {
                      hb_font_t *hb_font = ns_pango_font_get_hb_font (prev->analysis.font);
                  ...
                  switch (value)
                    {
                    case NS_PANGO_BASELINE_SHIFT_NONE:
                      entry->x_offset = 0;
                      entry->y_offset = 0;
                      break;
                    case NS_PANGO_BASELINE_SHIFT_SUPERSCRIPT:
                      entry->x_offset = superscript_x_offset;
                      entry->y_offset = superscript_y_offset;
                      break;
                    case NS_PANGO_BASELINE_SHIFT_SUBSCRIPT:
              ...
              for (t = state->baseline_shifts; t; t = t->next)
                {
                  ...
                  if (attr->start_index == entry->attr->start_index &&
                      ...
                      ((NsPangoAttrInt *)attr)->value == ((NsPangoAttrInt *)entry->attr)->value)
                    {
...
static void
apply_baseline_shift (NsPangoLayoutLine *line,
                       ParaBreakState  *state)
{
  int y_offset = 0;
  NsPangoItem *prev = NULL;
  ...
  for (GSList *l = line->runs; l; l = l->next)
    {
      ...
      language = hb_language_from_string (ns_pango_language_to_string (item->analysis.language), -1);
      hb_ot_tags_from_script_and_language (script, language,
                                           &script_count, script_tags,
                                           &lang_count, lang_tags);

      if (item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_CENTERED_BASELINE)
...
static void
ns_pango_layout_line_postprocess (NsPangoLayoutLine *line,
                               ParaBreakState  *state,
                               gboolean         wrapped)
{
  ...
  if (G_UNLIKELY (state->line_width >= 0 &&
                  should_ellipsize_current_line (line->layout, state)))
    {
      ...
      if (ns_pango_context_get_round_glyph_positions (line->layout->context))
        shape_flags |= NS_PANGO_SHAPE_ROUND_POSITIONS;

      ellipsized = _ns_pango_layout_line_ellipsize (line, state->attrs, shape_flags, state->line_width);
    }

  /* Now convert logical to visual order */
  ns_pango_layout_line_reorder (line);
  ...
  if (line->layout->justify && (wrapped || ellipsized || line->layout->justify_last_line))
    {
      ...
      if (state->remaining_width < 0)
        state->remaining_width = state->line_width - ns_pango_layout_line_get_width (line);
...
static void
ns_pango_layout_get_item_properties (NsPangoItem      *item,
                                  ItemProperties *properties)
{
  ...
  while (tmp_list)
    {
      NsPangoAttribute *attr = tmp_list->data;

      switch ((int) attr->klass->type)
        {
        case NS_PANGO_ATTR_UNDERLINE:
          switch ((NsPangoUnderline)(((NsPangoAttrInt *)attr)->value))
            {
        ...
        case NS_PANGO_ATTR_OVERLINE:
          switch ((NsPangoOverline)(((NsPangoAttrInt *)attr)->value))
            {
        ...
        case NS_PANGO_ATTR_STRIKETHROUGH:
          properties->strikethrough = ((NsPangoAttrInt *)attr)->value;
          break;
        ...
        case NS_PANGO_ATTR_SHAPE:
          ...
          properties->shape_logical_rect = &((NsPangoAttrShape *)attr)->logical_rect;
          properties->shape_ink_rect = &((NsPangoAttrShape *)attr)->ink_rect;
          break;

        case NS_PANGO_ATTR_LINE_HEIGHT:
          properties->line_height = ((NsPangoAttrFloat *)attr)->value;
          break;

        case NS_PANGO_ATTR_ABSOLUTE_LINE_HEIGHT:
          properties->absolute_line_height = ((NsPangoAttrInt *)attr)->value;
          break;

        case NS_PANGO_ATTR_SHOW:
          properties->showing_space = (((NsPangoAttrInt *)attr)->value & NS_PANGO_SHOW_SPACES) != 0;
          break;
...
static int
next_cluster_start (NsPangoGlyphString *gs,
                    int               cluster_start)
{
  int i;
...
static int
cluster_width (NsPangoGlyphString *gs,
               int               cluster_start)
{
  int i;
  int width;
...
static inline void
offset_y (NsPangoLayoutIter *iter,
          int             *y)
{
 ...
 */
static void
update_cluster (NsPangoLayoutIter *iter,
                int              cluster_start_index)
{
  char             *cluster_text;
  NsPangoGlyphString *gs;
  int               cluster_length;
  ...
  iter->cluster_num_chars = ns_pango_utf8_strlen (cluster_text, cluster_length);
...
static void
update_run (NsPangoLayoutIter *iter,
            int              run_start_index)
{
  ...
  if (iter->run)
    {
      iter->run_width = ns_pango_glyph_string_get_width (iter->run->glyphs);
...
/**
 * ns_pango_layout_iter_copy:
 * @iter: (nullable): a `NsPangoLayoutIter`
 *
 * Copies a `NsPangoLayoutIter`.
 *
 * Returns: (transfer full) (nullable): the newly allocated `NsPangoLayoutIter`
 *
 * Since: 1.20
 */
NsPangoLayoutIter *
ns_pango_layout_iter_copy (NsPangoLayoutIter *iter)
{
  NsPangoLayoutIter *new;

  if (iter == NULL)
    return NULL;

  new = g_slice_new (NsPangoLayoutIter);

  new->layout = g_object_ref (iter->layout);
  new->line_list_link = iter->line_list_link;
  new->line = iter->line;
  ns_pango_layout_line_ref (new->line);
...
G_DEFINE_BOXED_TYPE (NsPangoLayoutIter, ns_pango_layout_iter,
                     ns_pango_layout_iter_copy,
                     ns_pango_layout_iter_free);

/**
 * ns_pango_layout_get_iter:
 * @layout: a `NsPangoLayout`
 *
 * Returns an iterator to iterate over the visual extents of the layout.
 *
 * Returns: (transfer full): the new `NsPangoLayoutIter`
 */
NsPangoLayoutIter*
ns_pango_layout_get_iter (NsPangoLayout *layout)
{
  NsPangoLayoutIter *iter;

  g_return_val_if_fail (NS_PANGO_IS_LAYOUT (layout), NULL);

  iter = g_slice_new (NsPangoLayoutIter);
...
void
_ns_pango_layout_get_iter (NsPangoLayout    *layout,
                        NsPangoLayoutIter*iter)
{
  int run_start_index;
  ...
  ns_pango_layout_line_ref (iter->line);
  ...
  if (layout->width == -1)
    {
      ...
      ns_pango_layout_get_extents_internal (layout,
                                         NULL,
                                         &logical_rect,
                                         &iter->line_extents);
...
void
_ns_pango_layout_iter_destroy (NsPangoLayoutIter *iter)
{
  if (iter == NULL)
    return;

  g_free (iter->line_extents);
  ns_pango_layout_line_unref (iter->line);
  g_object_unref (iter->layout);
}

/**
 * ns_pango_layout_iter_free:
 * @iter: (nullable): a `NsPangoLayoutIter`, may be %NULL
 *
 * Frees an iterator that's no longer in use.
 **/
void
ns_pango_layout_iter_free (NsPangoLayoutIter *iter)
{
  if (iter == NULL)
    return;

  _ns_pango_layout_iter_destroy (iter);
  g_slice_free (NsPangoLayoutIter, iter);
}

/**
 * ns_pango_layout_iter_get_index:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
int
ns_pango_layout_iter_get_index (NsPangoLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return 0;
 ...
 */
NsPangoLayoutRun*
ns_pango_layout_iter_get_run (NsPangoLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return NULL;

  ns_pango_layout_line_leaked (iter->line);

  return iter->run;
}

/**
 * ns_pango_layout_iter_get_run_readonly:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
NsPangoLayoutRun*
ns_pango_layout_iter_get_run_readonly (NsPangoLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return NULL;
...
static NsPangoLayoutLine*
_ns_pango_layout_iter_get_line (NsPangoLayoutIter *iter)
{
  return iter->line;
}

static NsPangoLayoutRun *
_ns_pango_layout_iter_get_run (NsPangoLayoutIter *iter)
{
  return iter->run;
}

/**
 * ns_pango_layout_iter_get_line:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
NsPangoLayoutLine*
ns_pango_layout_iter_get_line (NsPangoLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return NULL;
...
/**
 * ns_pango_layout_iter_get_line_readonly:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
NsPangoLayoutLine*
ns_pango_layout_iter_get_line_readonly (NsPangoLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return NULL;
...
/**
 * ns_pango_layout_iter_at_last_line:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
gboolean
ns_pango_layout_iter_at_last_line (NsPangoLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return FALSE;
...
/**
 * ns_pango_layout_iter_get_layout:
 * @iter: a `NsPangoLayoutIter`
 *
 * Gets the layout associated with a `NsPangoLayoutIter`.
 *
 ...
 */
NsPangoLayout*
ns_pango_layout_iter_get_layout (NsPangoLayoutIter *iter)
{
...
static gboolean
line_is_terminated (NsPangoLayoutIter *iter)
{
  ...
  if (iter->line_list_link->next)
    {
      NsPangoLayoutLine *next_line = iter->line_list_link->next->data;
 ...
 */
static gboolean
next_nonempty_line (NsPangoLayoutIter *iter,
                    gboolean         include_terminators)
{
  ...
  while (TRUE)
    {
      result = ns_pango_layout_iter_next_line (iter);
      if (!result)
        break;
 ...
 */
static gboolean
next_nonempty_run (NsPangoLayoutIter *iter,
                    gboolean         include_terminators)
{
  gboolean result;
  ...
  while (TRUE)
    {
      result = ns_pango_layout_iter_next_run (iter);
      if (!result)
        break;
...
/* Like ns_pango_layout_next_cluster(), but if @include_terminators
 * is set, includes the fake runs/clusters for empty lines.
 * (But not positions introduced by line wrapping).
 */
static gboolean
next_cluster_internal (NsPangoLayoutIter *iter,
                       gboolean         include_terminators)
{
  NsPangoGlyphString *gs;
  int               next_start;
...
/**
 * ns_pango_layout_iter_next_char:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
gboolean
ns_pango_layout_iter_next_char (NsPangoLayoutIter *iter)
{
  const char *text;
...
/**
 * ns_pango_layout_iter_next_cluster:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
gboolean
ns_pango_layout_iter_next_cluster (NsPangoLayoutIter *iter)
{
  return next_cluster_internal (iter, FALSE);
}

/**
 * ns_pango_layout_iter_next_run:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
gboolean
ns_pango_layout_iter_next_run (NsPangoLayoutIter *iter)
{
  ...
  if (iter->run == NULL)
    return ns_pango_layout_iter_next_line (iter);
...
/**
 * ns_pango_layout_iter_next_line:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
gboolean
ns_pango_layout_iter_next_line (NsPangoLayoutIter *iter)
{
  GSList *next_link;
...
/**
 * ns_pango_layout_iter_get_char_extents:
 * @iter: a `NsPangoLayoutIter`
 ...
 */
void
ns_pango_layout_iter_get_char_extents (NsPangoLayoutIter *iter,
                                    NsPangoRectangle  *logical_rect)
{
  NsPangoRectangle cluster_rect;
  int            x0, x1;
  ...
  ns_pango_layout_iter_get_cluster_extents (iter, NULL, &cluster_rect);
...
/**
 * ns_pango_layout_iter_get_cluster_extents:
 * @iter: a `NsPangoLayoutIter`
 ...
 */
void
ns_pango_layout_iter_get_cluster_extents (NsPangoLayoutIter *iter,
                                       NsPangoRectangle  *ink_rect,
                                       NsPangoRectangle  *logical_rect)
{
  if (ITER_IS_INVALID (iter))
  ...
  if (iter->run == NULL)
    {
       ...
       * same extents
       */
      ns_pango_layout_iter_get_run_extents (iter, ink_rect, logical_rect);
      return;
    }

  ns_pango_glyph_string_extents_range (iter->run->glyphs,
                                    iter->cluster_start,
...
/**
 * ns_pango_layout_iter_get_run_extents:
 * @iter: a `NsPangoLayoutIter`
 * @ink_rect: (out) (optional): rectangle to fill with ink extents
 * @logical_rect: (out) (optional): rectangle to fill with logical extents
 *
 ...
 */
void
ns_pango_layout_iter_get_run_extents (NsPangoLayoutIter *iter,
                                   NsPangoRectangle  *ink_rect,
                                   NsPangoRectangle  *logical_rect)
{
  ...
  if (iter->run)
    {
      ns_pango_layout_run_get_extents_and_height (iter->run, ink_rect, logical_rect, NULL, NULL);
      ...
      if (iter->line->runs)
        {
          ...
          NsPangoLayoutRun *run = g_slist_last (iter->line->runs)->data;
          ns_pango_layout_run_get_extents_and_height (run, ink_rect, logical_rect, NULL, NULL);
        }
      else
        {
          NsPangoRectangle r;

          ns_pango_layout_get_empty_extents_and_height_at_index (iter->layout, 0, &r, FALSE, NULL);
...
/**
 * ns_pango_layout_iter_get_line_extents:
 * @iter: a `NsPangoLayoutIter`
 * @ink_rect: (out) (optional): rectangle to fill with ink extents
 * @logical_rect: (out) (optional): rectangle to fill with logical extents
 *
 ...
 * of the entire `NsPangoLayout`). Thus the extents returned by this
 * function will be the same width/height but not at the same x/y
 * as the extents returned from [method@Pango.LayoutLine.get_extents].
 */
void
ns_pango_layout_iter_get_line_extents (NsPangoLayoutIter *iter,
                                    NsPangoRectangle  *ink_rect,
                                    NsPangoRectangle  *logical_rect)
{
  const Extents *ext;
...
/**
 * ns_pango_layout_iter_get_line_yrange:
 * @iter: a `NsPangoLayoutIter`
 * @y0_: (out) (optional): start of line
 * @y1_: (out) (optional): end of line
 *
 * Divides the vertical space in the `NsPangoLayout` being iterated over
 ...
 */
void
ns_pango_layout_iter_get_line_yrange (NsPangoLayoutIter *iter,
                                   int             *y0,
                                   int             *y1)
{
  const Extents *ext;
...
/**
 * ns_pango_layout_iter_get_baseline:
 * @iter: a `NsPangoLayoutIter`
 *
 ...
 */
int
ns_pango_layout_iter_get_baseline (NsPangoLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return 0;
...
/**
 * ns_pango_layout_iter_get_run_baseline:
 * @iter: a `NsPangoLayoutIter`
 * 
 ...
 */
int
ns_pango_layout_iter_get_run_baseline (NsPangoLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return 0;
...
/**
 * ns_pango_layout_iter_get_layout_extents:
 * @iter: a `NsPangoLayoutIter`
 * @ink_rect: (out) (optional): rectangle to fill with ink extents
 * @logical_rect: (out) (optional): rectangle to fill with logical extents
 *
 * Obtains the extents of the `NsPangoLayout` being iterated over.
 */
void
ns_pango_layout_iter_get_layout_extents (NsPangoLayoutIter *iter,
                                      NsPangoRectangle  *ink_rect,
                                      NsPangoRectangle  *logical_rect)
{
  if (ITER_IS_INVALID (iter))
  ...
  ns_pango_layout_get_extents (iter->layout, ink_rect, logical_rect);
}
