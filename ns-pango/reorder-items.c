 ...
 *     pango-layout.c:ns_pango_layout_line_reorder().
 */

static GList *reorder_items_recurse (GList *items, int n_items);

/**
 * ns_pango_reorder_items:
 * @items: (element-type Pango.Item): a `GList` of `NsPangoItem`
 *   in logical order.
 *
 ...
 *   of `NsPangoItem` structures in visual order.
 */
GList *
ns_pango_reorder_items (GList *items)
{
                       ...
                       int    n_items)
{
  ...
  for (i=0; i<n_items; i++)
    {
      NsPangoItem *item = tmp_list->data;
