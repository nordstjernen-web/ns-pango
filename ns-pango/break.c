...
#define EMOJI(wc) (_ns_pango_Is_Emoji_Base_Character (wc))
               ...
               int            attrs_len G_GNUC_UNUSED)
{
  ...
  prev_wc = 0;
  prev_prev_wc = 0;
  prev_script = NS_PANGO_SCRIPT_COMMON;
  prev_jamo = NO_JAMO;
  ...
  for (i = 0; !done ; i++)
	...
	_ns_pango_Is_Emoji_Extended_Pictographic (wc);
	...
	switch ((int)type)
	  ...
	  case G_UNICODE_OTHER_LETTER:
	    ...
	    if (_ns_pango_is_Consonant_Preceding_Repha (wc) ||
		_ns_pango_is_Consonant_Prefixed (wc))
	      GB_type = GB_Prepend;
	    break;
	...
	if (met_Extended_Pictographic)
	  {
	    ...
	    else if (_ns_pango_Is_Emoji_Extended_Pictographic (prev_wc) &&
		     GB_type == GB_ZWJ)
	      met_Extended_Pictographic = TRUE;
	...
	if (met_InCB_Consonant || met_InCB_Consonant_Linker)
	  { /* Rule GB9c */
		...
		_ns_pango_is_Indic_Conjunct_Break_Consonant(wc))
	      {
	    ...
	    if (!(_ns_pango_is_Indic_Conjunct_Break_Extend(wc) ||
		  _ns_pango_is_Indic_Conjunct_Break_Linker(wc)))
	      {
	...
	if (is_grapheme_boundary)
	    ...
	    if (_ns_pango_is_Virama (prev_wc) ||
		_ns_pango_is_Vowel_Dependent (prev_wc))
	...
	prev_GB_type = GB_type;
      ...
      script = (NsPangoScript)g_unichar_get_script (wc);
	...
	if (is_grapheme_boundary ||
	    G_UNLIKELY(wc >=0x1F1E6 && wc <=0x1F1FF)) /* Rules WB3 and WB4 */
	  {
	    ...
	    if (script == NS_PANGO_SCRIPT_KATAKANA)
	      WB_type = WB_Katakana;

	    if (script == NS_PANGO_SCRIPT_HEBREW && type == G_UNICODE_OTHER_LETTER)
		...
		Alphabetic:
		  if (break_type != G_UNICODE_BREAK_COMPLEX_CONTEXT && script != NS_PANGO_SCRIPT_HIRAGANA)
		    WB_type = WB_ALetter; /* ALetter */
		  break;
                default:
                  break;
...
#undef IS_OTHER_TERM
	  ...
	  FALSE)
	{
	  ...
	  if ((prev_break_type == G_UNICODE_BREAK_ALPHABETIC ||
	      ...
	      !_ns_pango_is_EastAsianWide (wc))
	    break_op = BREAK_PROHIBITED;

	  if (prev_break_type == G_UNICODE_BREAK_CLOSE_PARANTHESIS &&
	      !_ns_pango_is_EastAsianWide (prev_wc)&&
	      (break_type == G_UNICODE_BREAK_ALPHABETIC ||
	  ...
	  /* Rule LB19a */
	  if (!_ns_pango_is_EastAsianWide(prev_wc) &&
	      break_type == G_UNICODE_BREAK_QUOTATION)
	    break_op = BREAK_PROHIBITED;

	  if (break_type == G_UNICODE_BREAK_QUOTATION &&
	      (!_ns_pango_is_EastAsianWide(next_wc) || done))
	    break_op = BREAK_PROHIBITED;
	  ...
	  if ((prev_LB_i == 0 || !_ns_pango_is_EastAsianWide(prev_prev_wc)) &&
	  ...
	  if (break_type == G_UNICODE_BREAK_COMBINING_MARK ||
	    ...
	    {
	      if (_ns_pango_Is_Emoji_Extended_Pictographic (wc) &&
	...
	if (last_sentence_start != -1 &&
...

      {
        ...
        switch ((int)prev_script)
          {
          case NS_PANGO_SCRIPT_COMMON:
            insert_hyphens = prev_wc == 0x00ad;
            break;
          case NS_PANGO_SCRIPT_HAN:
          case NS_PANGO_SCRIPT_HANGUL:
          case NS_PANGO_SCRIPT_HIRAGANA:
          case NS_PANGO_SCRIPT_KATAKANA:
	      ...
	      int                  attrs_len)
{
  switch (analysis->script)
    {
    case NS_PANGO_SCRIPT_ARABIC:
      break_arabic (item_text, item_length, analysis, attrs, attrs_len);
      break;

    case NS_PANGO_SCRIPT_DEVANAGARI:
    case NS_PANGO_SCRIPT_BENGALI:
    case NS_PANGO_SCRIPT_GURMUKHI:
    case NS_PANGO_SCRIPT_GUJARATI:
    case NS_PANGO_SCRIPT_ORIYA:
    case NS_PANGO_SCRIPT_TAMIL:
    case NS_PANGO_SCRIPT_TELUGU:
    case NS_PANGO_SCRIPT_KANNADA:
    case NS_PANGO_SCRIPT_MALAYALAM:
    case NS_PANGO_SCRIPT_SINHALA:
      break_indic (item_text, item_length, analysis, attrs, attrs_len);
      break;

    case NS_PANGO_SCRIPT_THAI:
      break_thai (item_text, item_length, analysis, attrs, attrs_len);
      break;

    case NS_PANGO_SCRIPT_LATIN:
                     ...
                     int            log_attrs_len)
{
  ...
  do
    {
      const NsPangoAttribute *attr = ns_pango_attr_iterator_get (&iter, NS_PANGO_ATTR_ALLOW_BREAKS);

      if (!attr)
        continue;

      if (!((NsPangoAttrInt*)attr)->value)
        {
          int start, end;
              ...
              int            log_attrs_len)
{
  ...
  do
    {
      const NsPangoAttribute *attr = ns_pango_attr_iterator_get (&iter, NS_PANGO_ATTR_WORD);
      int start, end;
      int start_pos, end_pos;
      int pos;
                  ...
                  int            log_attrs_len)
{
  ...
  do
    {
      const NsPangoAttribute *attr = ns_pango_attr_iterator_get (&iter, NS_PANGO_ATTR_SENTENCE);
      int start, end;
      int start_pos, end_pos;
      int pos;
                ...
                int            log_attrs_len)
{
  ...
  do {
    const NsPangoAttribute *attr = ns_pango_attr_iterator_get (&iter, NS_PANGO_ATTR_INSERT_HYPHENS);

    if (attr && ((NsPangoAttrInt*)attr)->value == 0)
      {
        int start, end;
        int start_pos, end_pos;
        int pos;
...
break_attrs (const char   *text,
             ...
             int           log_attrs_len)
{
  NsPangoAttrList allow_breaks;
  NsPangoAttrList words;
  NsPangoAttrList sentences;
  NsPangoAttrList hyphens;
  GSList *l;
  gboolean tailored = FALSE;

  _ns_pango_attr_list_init (&allow_breaks);
  _ns_pango_attr_list_init (&words);
  _ns_pango_attr_list_init (&sentences);
  _ns_pango_attr_list_init (&hyphens);

  for (l = attributes; l; l = l->next)
    {
      NsPangoAttribute *attr = l->data;

      if (attr->klass->type == NS_PANGO_ATTR_ALLOW_BREAKS)
        ns_pango_attr_list_insert (&allow_breaks, ns_pango_attribute_copy (attr));
      else if (attr->klass->type == NS_PANGO_ATTR_WORD)
        ns_pango_attr_list_insert (&words, ns_pango_attribute_copy (attr));
      else if (attr->klass->type == NS_PANGO_ATTR_SENTENCE)
        ns_pango_attr_list_insert (&sentences, ns_pango_attribute_copy (attr));
      else if (attr->klass->type == NS_PANGO_ATTR_INSERT_HYPHENS)
        ns_pango_attr_list_insert (&hyphens, ns_pango_attribute_copy (attr));
    }
  ...
  _ns_pango_attr_list_destroy (&allow_breaks);
  _ns_pango_attr_list_destroy (&words);
  _ns_pango_attr_list_destroy (&sentences);
  _ns_pango_attr_list_destroy (&hyphens);
...
/**
 * ns_pango_default_break:
 * @text: text to break. Must be valid UTF-8
 * @length: length of text in bytes (may be -1 if @text is nul-terminated)
 * @analysis: (nullable): a `NsPangoAnalysis` structure for the @text
 ...
 * @length: length of @text in bytes (may be -1 if @text is nul-terminated)
 * @analysis: `NsPangoAnalysis` structure for @text
 ...
 */
void
ns_pango_break (const char    *text,
             gint           length,
             NsPangoAnalysis *analysis,
             NsPangoLogAttr  *attrs,
             int            attrs_len)
...
/**
 * ns_pango_tailor_break:
 * @text: text to process. Must be valid UTF-8
 * @length: length in bytes of @text
 * @analysis: `NsPangoAnalysis` for @text
 ...
 *   `NsPangoLogAttr` per character in @text, plus one extra, to be filled in
 ...
 */
void
ns_pango_tailor_break (const char    *text,
                    int            length,
                    NsPangoAnalysis *analysis,
                    int            offset,
                    NsPangoLogAttr  *attrs,
                    int            attrs_len)
{
  NsPangoLogAttr *start = attrs;
  NsPangoLogAttr attr_before = *start;
 ...
 * @text: text to break. Must be valid UTF-8
 * @length: length of text in bytes (may be -1 if @text is nul-terminated)
 * @attr_list: `NsPangoAttrList` to apply
 ...
 */
void
ns_pango_attr_break (const char    *text,
                  int            length,
                  ...
                  int            attrs_len)
{
  ...
  attributes = ns_pango_attr_list_get_attributes (attr_list);
 ...
 * @attrs_len: (in): length of @attrs array
 *
 * Computes a `NsPangoLogAttr` for each character in @text.
 *
 * The @attrs array must have one `NsPangoLogAttr` for
 ...
 */
void
ns_pango_get_log_attrs (const char    *text,
                     int            length,
                     int            level,
...
{
  int chars_broken;
  NsPangoAnalysis analysis = { NULL };
  NsPangoScriptIter iter;
  ...
  ns_pango_default_break (text, length, &analysis, attrs, attrs_len);

  chars_broken = 0;

  _ns_pango_script_iter_init (&iter, text, length);
  do
    {
      ...
      ns_pango_script_iter_get_range (&iter, &run_start, &run_end, &script);
      analysis.script = script;

      chars_in_range = ns_pango_utf8_strlen (run_start, run_end - run_start);

      ns_pango_tailor_break (run_start,
                          run_end - run_start,
                          &analysis,
                          -1,
                          attrs + chars_broken,
  ...
  while (ns_pango_script_iter_next (&iter));
  _ns_pango_script_iter_fini (&iter);

  if (chars_broken + 1 > attrs_len)
    g_warning ("ns_pango_get_log_attrs: attrs_len should have been at least %d, but was %d.  Expect corrupted memory.",
               chars_broken + 1,
               attrs_len);
}
