/* Pango
 * pango-glyph-item.c: Pair of NsPangoItem and a glyph string
 *
 ...
 *
 * This function is similar in function to ns_pango_item_split() (and uses
 * it internally.)
 *
 * Returns: (transfer full) (nullable): the newly allocated item
 *   representing text before @split_index, which should be freed
 *   with ns_pango_glyph_item_free().
 *
 * Since: 1.2
 */
NsPangoGlyphItem *
ns_pango_glyph_item_split (NsPangoGlyphItem *orig,
			const char     *text,
			int             split_index)
{
  NsPangoGlyphItem *new;
  int i;
  int num_glyphs;
  int num_remaining;
  int split_offset;
  ...
  new = g_slice_new (NsPangoGlyphItem);
  split_offset = g_utf8_pointer_to_offset (text + orig->item->offset,
					   text + orig->item->offset + split_index);
  new->item = ns_pango_item_split (orig->item, split_index, split_offset);

  new->glyphs = ns_pango_glyph_string_new ();
  ns_pango_glyph_string_set_size (new->glyphs, num_glyphs);

  if (LTR (orig))
    {
      memcpy (new->glyphs->glyphs, orig->glyphs->glyphs, num_glyphs * sizeof (NsPangoGlyphInfo));
      ...
      memmove (orig->glyphs->glyphs, orig->glyphs->glyphs + num_glyphs,
	       num_remaining * sizeof (NsPangoGlyphInfo));
      for (i = num_glyphs; i < orig->glyphs->num_glyphs; i++)
	orig->glyphs->log_clusters[i - num_glyphs] = orig->glyphs->log_clusters[i] - split_index;
    }
  else
    {
      memcpy (new->glyphs->glyphs, orig->glyphs->glyphs + num_remaining, num_glyphs * sizeof (NsPangoGlyphInfo));
  ...
  ns_pango_glyph_string_set_size (orig->glyphs, orig->glyphs->num_glyphs - num_glyphs);
...
/**
 * ns_pango_glyph_item_copy:
 * @orig: (nullable): a `NsPangoGlyphItem`
 *
 * Make a deep copy of an existing `NsPangoGlyphItem` structure.
 *
 * Returns: (transfer full) (nullable): the newly allocated `NsPangoGlyphItem`
 *
 * Since: 1.20
 */
NsPangoGlyphItem *
ns_pango_glyph_item_copy  (NsPangoGlyphItem *orig)
{
  NsPangoGlyphItem *result;

  if (orig == NULL)
    return NULL;

  result = g_slice_new (NsPangoGlyphItem);

  result->item = ns_pango_item_copy (orig->item);
  result->glyphs = ns_pango_glyph_string_copy (orig->glyphs);
...
/**
 * ns_pango_glyph_item_free:
 * @glyph_item: (nullable): a `NsPangoGlyphItem`
 *
 * Frees a `NsPangoGlyphItem` and resources to which it points.
 *
 * Since: 1.6
 */
void
ns_pango_glyph_item_free  (NsPangoGlyphItem *glyph_item)
{
  if (glyph_item == NULL)
    return;

  if (glyph_item->item)
    ns_pango_item_free (glyph_item->item);
  if (glyph_item->glyphs)
    ns_pango_glyph_string_free (glyph_item->glyphs);

  g_slice_free (NsPangoGlyphItem, glyph_item);
}

G_DEFINE_BOXED_TYPE (NsPangoGlyphItem, ns_pango_glyph_item,
                     ns_pango_glyph_item_copy,
                     ns_pango_glyph_item_free);


/**
 * ns_pango_glyph_item_iter_copy:
 * @orig: (nullable): a `NsPangoGlyphItem`Iter
 *
 * Make a shallow copy of an existing `NsPangoGlyphItemIter` structure.
 *
 * Returns: (transfer full) (nullable): the newly allocated `NsPangoGlyphItemIter`
 *
 * Since: 1.22
 */
NsPangoGlyphItemIter *
ns_pango_glyph_item_iter_copy (NsPangoGlyphItemIter *orig)
{
  NsPangoGlyphItemIter *result;

  if (orig == NULL)
    return NULL;

  result = g_slice_new (NsPangoGlyphItemIter);
...
/**
 * ns_pango_glyph_item_iter_free:
 * @iter: (nullable): a `NsPangoGlyphItemIter`
 *
 * Frees a `NsPangoGlyphItem`Iter.
 *
 * Since: 1.22
 */
void
ns_pango_glyph_item_iter_free  (NsPangoGlyphItemIter *iter)
{
  if (iter == NULL)
    return;

  g_slice_free (NsPangoGlyphItemIter, iter);
}

G_DEFINE_BOXED_TYPE (NsPangoGlyphItemIter, ns_pango_glyph_item_iter,
                     ns_pango_glyph_item_iter_copy,
                     ns_pango_glyph_item_iter_free)

/**
 * ns_pango_glyph_item_iter_next_cluster:
 * @iter: a `NsPangoGlyphItemIter`
 *
 * Advances the iterator to the next cluster in the glyph item.
 *
 * See `NsPangoGlyphItemIter` for details of cluster orders.
 *
 ...
 */
gboolean
ns_pango_glyph_item_iter_next_cluster (NsPangoGlyphItemIter *iter)
{
  int glyph_index = iter->end_glyph;
  NsPangoGlyphString *glyphs = iter->glyph_item->glyphs;
  int cluster;
  NsPangoItem *item = iter->glyph_item->item;
	  ...
	  if (glyphs->log_clusters[glyph_index] > cluster)
	    {
	      ...
	      iter->end_char += ns_pango_utf8_strlen (iter->text + iter->start_index,
 ...
 */
gboolean
ns_pango_glyph_item_iter_prev_cluster (NsPangoGlyphItemIter *iter)
{
	  ...
	  if (glyphs->log_clusters[glyph_index] < cluster)
	    {
	      ...
	      iter->start_char -= ns_pango_utf8_strlen (iter->text + iter->start_index,
						 iter->end_index - iter->start_index);
	      break;
	    }
	}
...
/**
 * ns_pango_glyph_item_iter_init_start:
 * @iter: a `NsPangoGlyphItemIter`
 * @glyph_item: the glyph item to iterate over
 * @text: text corresponding to the glyph item
 *
 * Initializes a `NsPangoGlyphItemIter` structure to point to the
 ...
 */
gboolean
ns_pango_glyph_item_iter_init_start (NsPangoGlyphItemIter  *iter,
				  ...
				  const char          *text)
{
  ...
  return ns_pango_glyph_item_iter_next_cluster (iter);
}

/**
 * ns_pango_glyph_item_iter_init_end:
 * @iter: a `NsPangoGlyphItemIter`
 * @glyph_item: the glyph item to iterate over
 * @text: text corresponding to the glyph item
 *
 ...
 */
gboolean
ns_pango_glyph_item_iter_init_end (NsPangoGlyphItemIter  *iter,
				NsPangoGlyphItem      *glyph_item,
				const char          *text)
{
  iter->glyph_item = glyph_item;
  ...
  /* Advance onto the first cluster of the glyph item */
  return ns_pango_glyph_item_iter_prev_cluster (iter);
}
 ...
 */
static void
append_attrs (NsPangoGlyphItem *glyph_item,
	      GSList         *attrs)
{
...
/* Make a deep copy of a GSList of NsPangoAttribute
 */
static GSList *
attr_slist_copy (GSList *attrs)
{
  ...
  for (tmp_list = new_attrs; tmp_list; tmp_list = tmp_list->next)
    tmp_list->data = ns_pango_attribute_copy (tmp_list->data);
 ...
 */
static NsPangoGlyphItem *
split_before_cluster_start (ApplyAttrsState *state)
{
  NsPangoGlyphItem *split_item;
  int split_len = state->iter.start_index - state->iter.glyph_item->item->offset;

  split_item = ns_pango_glyph_item_split (state->iter.glyph_item, state->iter.text, split_len);
...
/**
 * ns_pango_glyph_item_apply_attrs:
 * @glyph_item: (transfer full): a shaped item
 * @text: text that @list applies to
 * @list: a `NsPangoAttrList`
 *
 * Splits a shaped item (`NsPangoGlyphItem`) into multiple items based
 ...
 */
GSList *
ns_pango_glyph_item_apply_attrs (NsPangoGlyphItem   *glyph_item,
			      const char       *text,
			      NsPangoAttrList    *list)
{
   ...
   */
  _ns_pango_attr_list_get_iterator (list, &iter);
  do
    {
      ns_pango_attr_iterator_range (&iter, &range_start, &range_end);
      if (range_end > glyph_item->item->offset)
	break;
    }
  while (ns_pango_attr_iterator_next (&iter));

  state.segment_attrs = ns_pango_attr_iterator_get_attrs (&iter);

  is_ellipsis = (glyph_item->item->analysis.flags & NS_PANGO_ANALYSIS_FLAG_IS_ELLIPSIS) != 0;
  ...
  for (have_cluster = ns_pango_glyph_item_iter_init_start (&state.iter, glyph_item, text);
       have_cluster;
       have_cluster = ns_pango_glyph_item_iter_next_cluster (&state.iter))
    {
      gboolean have_next;
      ...
      do
	{
	  ...
	  have_next = ns_pango_attr_iterator_next (&iter);
	  ns_pango_attr_iterator_range (&iter, &range_start, &range_end);
	  ...
	  state.segment_attrs = g_slist_concat (state.segment_attrs,
						ns_pango_attr_iterator_get_attrs (&iter));
	}
      while (have_next);
    }
...
/**
 * ns_pango_glyph_item_letter_space:
 ...
 */
void
ns_pango_glyph_item_letter_space (NsPangoGlyphItem *glyph_item,
			       const char     *text,
 ...
 *   character widths.
 *
 * Given a `NsPangoGlyphItem` and the corresponding text, determine the
 ...
 */
void
ns_pango_glyph_item_get_logical_widths (NsPangoGlyphItem *glyph_item,
				     const char     *text,
				     int            *logical_widths)
{
  NsPangoGlyphItemIter iter;
  ...
  for (has_cluster = ns_pango_glyph_item_iter_init_start (&iter, glyph_item, text);
       has_cluster;
       has_cluster = ns_pango_glyph_item_iter_next_cluster (&iter))
    {
