...
/**
 * ns_pango_gravity_to_rotation:
 * @gravity: gravity to query, should not be %NS_PANGO_GRAVITY_AUTO
 *
 * Converts a `NsPangoGravity` value to its natural rotation in radians.
 *
 ...
 */
double
ns_pango_gravity_to_rotation (NsPangoGravity gravity)
{
  double rotation;

  g_return_val_if_fail (gravity != NS_PANGO_GRAVITY_AUTO, 0);

  switch (gravity)
    {
      default:
      case NS_PANGO_GRAVITY_AUTO: /* shut gcc up */
      case NS_PANGO_GRAVITY_SOUTH:	rotation =  0;		break;
      case NS_PANGO_GRAVITY_NORTH:	rotation =  G_PI;	break;
      case NS_PANGO_GRAVITY_EAST:	rotation = -G_PI_2;	break;
      case NS_PANGO_GRAVITY_WEST:	rotation = +G_PI_2;	break;
    }
...
/**
 * ns_pango_gravity_get_for_matrix:
 ...
 * %NS_PANGO_GRAVITY_AUTO, or %NS_PANGO_GRAVITY_SOUTH if @matrix is %NULL
 *
 * Since: 1.16
 */
NsPangoGravity
ns_pango_gravity_get_for_matrix (const NsPangoMatrix *matrix)
{
  ...
  if (fabs (x) > fabs (y))
    gravity = x > 0 ? NS_PANGO_GRAVITY_WEST : NS_PANGO_GRAVITY_EAST;
  else
    gravity = y < 0 ? NS_PANGO_GRAVITY_NORTH : NS_PANGO_GRAVITY_SOUTH;
...
typedef enum
{
  NS_PANGO_VERTICAL_DIRECTION_NONE,
  NS_PANGO_VERTICAL_DIRECTION_TTB,
  NS_PANGO_VERTICAL_DIRECTION_BTT
} NsPangoVerticalDirection;
...
} NsPangoScriptProperties;

#define NONE NS_PANGO_VERTICAL_DIRECTION_NONE
#define TTB  NS_PANGO_VERTICAL_DIRECTION_TTB
#define BTT  NS_PANGO_VERTICAL_DIRECTION_BTT

#define LTR  NS_PANGO_DIRECTION_LTR
#define RTL  NS_PANGO_DIRECTION_RTL
#define WEAK NS_PANGO_DIRECTION_WEAK_LTR

#define S NS_PANGO_GRAVITY_SOUTH
#define E NS_PANGO_GRAVITY_EAST
#define W NS_PANGO_GRAVITY_WEST

const NsPangoScriptProperties script_properties[] =
  {				/* ISO 15924 code */
...
static NsPangoScriptProperties
get_script_properties (NsPangoScript script)
{
...
/**
 * ns_pango_gravity_get_for_script:
 * @script: `NsPangoScript` to query
 * @base_gravity: base gravity of the paragraph
 * @hint: orientation hint
 *
 * Returns the gravity to use in laying out a `NsPangoItem`.
 *
 * The gravity is determined based on the script, base gravity, and hint.
 *
 * If @base_gravity is %NS_PANGO_GRAVITY_AUTO, it is first replaced with the
 * preferred gravity of @script.  To get the preferred gravity of a script,
 * pass %NS_PANGO_GRAVITY_AUTO and %NS_PANGO_GRAVITY_HINT_STRONG in.
 *
 ...
 */
NsPangoGravity
ns_pango_gravity_get_for_script (NsPangoScript      script,
                              NsPangoGravity     base_gravity,
                              NsPangoGravityHint hint)
{
  NsPangoScriptProperties props = get_script_properties (script);

  return ns_pango_gravity_get_for_script_and_width (script, props.wide,
                                                 base_gravity, hint);
}

/**
 * ns_pango_gravity_get_for_script_and_width:
 * @script: `NsPangoScript` to query
 ...
 */
NsPangoGravity
ns_pango_gravity_get_for_script_and_width (NsPangoScript      script,
                                        gboolean         wide,
                                        NsPangoGravity     base_gravity,
                                        NsPangoGravityHint hint)
{
  ...
  if (G_UNLIKELY (base_gravity == NS_PANGO_GRAVITY_AUTO))
    base_gravity = props.preferred_gravity;

  vertical = NS_PANGO_GRAVITY_IS_VERTICAL (base_gravity);
  ...
  switch (hint)
    {
    default:
    case NS_PANGO_GRAVITY_HINT_NATURAL:
      if (props.vert_dir == NS_PANGO_VERTICAL_DIRECTION_NONE)
	return NS_PANGO_GRAVITY_SOUTH;
      if ((base_gravity   == NS_PANGO_GRAVITY_EAST) ^
	  (props.vert_dir == NS_PANGO_VERTICAL_DIRECTION_BTT))
	return NS_PANGO_GRAVITY_SOUTH;
      else
	return NS_PANGO_GRAVITY_NORTH;

    case NS_PANGO_GRAVITY_HINT_STRONG:
      return base_gravity;

    case NS_PANGO_GRAVITY_HINT_LINE:
      if ((base_gravity    == NS_PANGO_GRAVITY_EAST) ^
	  (props.horiz_dir == NS_PANGO_DIRECTION_RTL))
	return NS_PANGO_GRAVITY_SOUTH;
      else
	return NS_PANGO_GRAVITY_NORTH;
    }
