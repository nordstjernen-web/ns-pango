...
{
  NsPangoLayout *layout;		/* Layout being ellipsized */
  NsPangoAttrList *attrs;		/* Attributes used for itemization/shaping */
  ...
  NsPangoGlyphItem *ellipsis_run;	/* Run created to hold ellipsis */
  ...
  NsPangoAttrIterator *line_start_attr; /* Cached NsPangoAttrIterator for the start of the run */
  ...
  NsPangoAttrIterator *gap_start_attr; /* Attribute iterator pointing to a range containing
            ...
            NsPangoShapeFlags  shape_flags)
{
  ...
  if (attrs)
    state->attrs = ns_pango_attr_list_ref (attrs);
  else
    state->attrs = ns_pango_attr_list_new ();
  ...
  start_offset = ns_pango_utf8_strlen (line->layout->text,
				line->start_index);

  state->total_width = 0;
  for (l = line->runs, i = 0; l; l = l->next, i++)
    {
      NsPangoGlyphItem *run = l->data;
...
{
  ns_pango_attr_list_unref (state->attrs);
  if (state->line_start_attr)
    ns_pango_attr_iterator_destroy (state->line_start_attr);
  if (state->gap_start_attr)
    ns_pango_attr_iterator_destroy (state->gap_start_attr);
...
{
  NsPangoGlyphItemIter *run_iter = &iter->run_iter;
  NsPangoGlyphString *glyphs = run_iter->glyph_item->glyphs;
  int width = 0;
  int i;
...
line_iter_next_cluster (EllipsizeState *state,
			LineIter       *iter)
{
  if (!ns_pango_glyph_item_iter_next_cluster (&iter->run_iter))
    {
	...
	{
	  iter->run_index++;
	  ns_pango_glyph_item_iter_init_start (&iter->run_iter,
...
line_iter_prev_cluster (EllipsizeState *state,
			LineIter       *iter)
{
  if (!ns_pango_glyph_item_iter_prev_cluster (&iter->run_iter))
    {
      if (iter->run_index == 0)
	return FALSE;
      else
	{
	  iter->run_index--;
	  ns_pango_glyph_item_iter_init_end (&iter->run_iter,
					  state->run_info[iter->run_index].run,
					  state->layout->text);
	}
    }
 ...
 * This should be an addition to `NsPangoGlyphVisAttr`
 *
 */
	      ...
	      NsPangoAttrList  *attrs)
{
  ...
  items = ns_pango_itemize (state->layout->context, text, 0, strlen (text), attrs, NULL);
...
shape_ellipsis (EllipsizeState *state)
{
  ...
  GSList *l;
  NsPangoAttribute *fallback;
  const char *ellipsis_text;
  int len;
  int i;

  _ns_pango_attr_list_init (&attrs);

  /* Create/reset state->ellipsis_run
   */
  if (!state->ellipsis_run)
    {
      state->ellipsis_run = g_slice_new0 (NsPangoGlyphItem);
      state->ellipsis_run->glyphs = ns_pango_glyph_string_new ();
    }

  if (state->ellipsis_run->item)
    {
      ns_pango_item_free (state->ellipsis_run->item);
   ...
   * or super/subscripts
   */
  run_attrs = ns_pango_attr_iterator_get_attrs (state->gap_start_attr);
  ...
  fallback = ns_pango_attr_fallback_new (FALSE);
  fallback->start_index = 0;
  fallback->end_index = G_MAXINT;
  ns_pango_attr_list_insert (&attrs, fallback);
  ...
  if (!item->analysis.font ||
      !ns_pango_font_has_char (item->analysis.font,
                            g_utf8_get_char (ellipsis_text)))
    {
      ns_pango_item_free (item);

      /* Modify the fallback iter while it is inside the NsPangoAttrList; Don't try this at home
       */
      ((NsPangoAttrInt *)fallback)->value = TRUE;

      ellipsis_text = "...";
      item = itemize_text (state, ellipsis_text, &attrs);
    }

  _ns_pango_attr_list_destroy (&attrs);
  ...
  ns_pango_shape_with_flags (ellipsis_text, len,
                          ellipsis_text, len,
...
/* Helper function to advance a NsPangoAttrIterator to a particular
 * byte index.
 */
static void
advance_iterator_to (NsPangoAttrIterator *iter,
		     int                new_index)
{
  int start, end;
...
update_ellipsis_shape (EllipsizeState *state)
{
  ...
  /* Unfortunately, we can only advance NsPangoAttrIterator forward; so each
  ...
  if (!state->line_start_attr)
    {
      state->line_start_attr = ns_pango_attr_list_get_iterator (state->attrs);
  ...
  if (state->gap_start_attr)
    {
      ...
      ns_pango_attr_iterator_range (state->gap_start_attr, &start, &end);
  ...
  if (!state->gap_start_attr)
    {
      state->gap_start_attr = ns_pango_attr_iterator_copy (state->line_start_attr);
...
find_initial_span (EllipsizeState *state)
{
  ...
  switch (state->layout->ellipsize)
    {
    case NS_PANGO_ELLIPSIZE_NONE:
    default:
      g_assert_not_reached ();
    case NS_PANGO_ELLIPSIZE_START:
      state->gap_center = 0;
      break;
    case NS_PANGO_ELLIPSIZE_MIDDLE:
      state->gap_center = state->total_width / 2;
      break;
    case NS_PANGO_ELLIPSIZE_END:
  ...
  for (have_cluster = ns_pango_glyph_item_iter_init_start (run_iter, glyph_item, state->layout->text);
       have_cluster;
       have_cluster = ns_pango_glyph_item_iter_next_cluster (run_iter))
    {
...
fixup_ellipsis_run (EllipsizeState *state,
                    int             extra_width)
{
  NsPangoGlyphString *glyphs = state->ellipsis_run->glyphs;
  NsPangoItem *item = state->ellipsis_run->item;
  int level;
  int i;
  ...
  item->num_chars = ns_pango_utf8_strlen (state->layout->text + item->offset, item->length);
  ...
  item->analysis.flags |= NS_PANGO_ANALYSIS_FLAG_IS_ELLIPSIS;
}
...
{
  NsPangoGlyphItem *partial_start_run = NULL;
  NsPangoGlyphItem *partial_end_run = NULL;
  GSList *result = NULL;
  RunInfo *run_info;
  NsPangoGlyphItemIter *run_iter;
  int i;
  ...
  if (run_iter->end_char != run_info->run->item->num_chars)
    {
      ...
      run_info->run = ns_pango_glyph_item_split (run_info->run, state->layout->text,
  ...
  if (run_iter->start_char != 0)
    {
      partial_start_run = ns_pango_glyph_item_split (run_info->run, state->layout->text,
   ...
   */
  for (i = state->gap_start_iter.run_index; i <= state->gap_end_iter.run_index; i++)
    ns_pango_glyph_item_free (state->run_info[i].run);
 ...
 **/
gboolean
_ns_pango_layout_line_ellipsize (NsPangoLayoutLine *line,
			      NsPangoAttrList   *attrs,
                              NsPangoShapeFlags  shape_flags,
			      int              goal_width)
{
  ...
  g_return_val_if_fail (line->layout->ellipsize != NS_PANGO_ELLIPSIZE_NONE && goal_width >= 0, is_ellipsized);
