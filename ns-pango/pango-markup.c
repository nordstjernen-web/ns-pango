...
{
  g_slist_foreach (ot->attrs, (GFunc) ns_pango_attribute_destroy, NULL);
...
markup_data_close_tag (MarkupData *md)
{
  ...
  while (tmp_list != NULL)
    {
      NsPangoAttribute *a = tmp_list->data;
  ...
  if (ot->scale_level_delta != 0)
      ...
      else
	{
	   ...
	   */
	  a = ns_pango_attr_scale_new (scale_factor (ot->scale_level,
                  ...
                  if (md->attr_list != NULL)
                    {
                      ...
                      attr = ns_pango_attr_underline_new (NS_PANGO_UNDERLINE_LOW);
                      ...
                      ns_pango_attr_list_change (md->attr_list, attr);
                    }
...
static const GMarkupParser ns_pango_markup_parser = {
...
destroy_markup_data (MarkupData *md)
{
  ...
  g_slist_free_full (md->to_apply, (GDestroyNotify) ns_pango_attribute_destroy);
  if (md->text)
      g_string_free (md->text, TRUE);

  if (md->attr_list)
    ns_pango_attr_list_unref (md->attr_list);

  g_slice_free (MarkupData, md);
}

static GMarkupParseContext *
ns_pango_markup_parser_new_internal (char       accel_marker,
				  GError   **error,
				  gboolean   want_attr_list)
{
   ...
   */
  if (want_attr_list)
    md->attr_list = ns_pango_attr_list_new ();
  else
    md->attr_list = NULL;
  ...
  context = g_markup_parse_context_new (&ns_pango_markup_parser,
					0, md,
...
/**
 * ns_pango_parse_markup:
 * @markup_text: markup to parse (see the [Pango Markup](ns_pango_markup.html) docs)
 * @length: length of @markup_text, or -1 if nul-terminated
 * @accel_marker: character that precedes an accelerator, or 0 for none
 * @attr_list: (out) (optional): address of return location for a `NsPangoAttrList`
 ...
 *
 * See the [Pango Markup](ns_pango_markup.html) docs for details about the
 * supported markup.
 *
 ...
 **/
gboolean
ns_pango_parse_markup (const char                 *markup_text,
		    int                         length,
		    gunichar                    accel_marker,
		    NsPangoAttrList             **attr_list,
		    char                      **text,
		    ...
		    GError                    **error)
{
  ...
  context = ns_pango_markup_parser_new_internal (accel_marker,
                                              error,
                                              (attr_list != NULL));
  ...
  if (!ns_pango_markup_parser_finish (context,
                                   attr_list,
                                   text,
                                   accel_char,
                                   error))
 ...
 **/
GMarkupParseContext *
ns_pango_markup_parser_new (gunichar accel_marker)
{
  return ns_pango_markup_parser_new_internal (accel_marker, NULL, TRUE);
}

/**
 * ns_pango_markup_parser_finish:
 ...
 */
gboolean
ns_pango_markup_parser_finish (GMarkupParseContext   *context,
                            NsPangoAttrList        **attr_list,
                            char                 **text,
                            gunichar              *accel_char,
                            GError               **error)
{
  ...
  if (md->attr_list)
      ...
      while (tmp_list != NULL)
	{
	  ...
	  ns_pango_attr_list_insert (md->attr_list, attr);
...
{
  CHECK_NO_ATTRS("b");
  add_attribute (tag, ns_pango_attr_weight_new (NS_PANGO_WEIGHT_BOLD));
  return TRUE;
}
                     ...
                     const char            *size)
{
  ...
  add_attribute (tag, ns_pango_attr_scale_new (factor));
  if (tag)
		...
		GError **error)
{
  ...
  if (!_ns_pango_scan_int (&end, val) || *end != '\0')
    {
      g_set_error (error,
		   G_MARKUP_ERROR,
...
span_parse_color (const char *attr_name,
		  ...
		  GError **error)
{
  if (!ns_pango_color_parse_with_alpha (color, alpha, attr_val))
    {
      g_set_error (error,
		   G_MARKUP_ERROR,
		   G_MARKUP_ERROR_INVALID_CONTENT,
                  ...
                  GError     **error)
{
  ...
  if (_ns_pango_scan_int (&end, &int_val))
    {
		 ...
		 GError **error)
{
  ...
  if (!_ns_pango_parse_enum (type, attr_val, val, FALSE, &possible_values))
    {
      g_set_error (error,
		   G_MARKUP_ERROR,
		   G_MARKUP_ERROR_INVALID_CONTENT,
                  ...
                  GError     **error)
{
  ...
  if (!ns_pango_parse_flags (type, attr_val, val, &possible_values))
    {
      g_set_error (error,
                   G_MARKUP_ERROR,
                   G_MARKUP_ERROR_INVALID_CONTENT,
              ...
              int        *result)
{
  ...
  attr = attr_val;
  if (_ns_pango_scan_int (&attr, &n) && *attr == '\0')
    {
      *result = n;
      return TRUE;
    }
...
#define CHECK_ATTRIBUTE(var) CHECK_ATTRIBUTE2 (var, G_STRINGIFY (var))
  ...
  if (G_UNLIKELY (desc))
    {
      NsPangoFontDescription *parsed;

      parsed = ns_pango_font_description_from_string (desc);
      if (parsed)
	{
	  add_attribute (tag, ns_pango_attr_font_desc_new (parsed));
	  if (tag)
	    open_tag_set_absolute_font_size (tag, ns_pango_font_description_get_size (parsed));
	  ns_pango_font_description_free (parsed);
	}
    }

  if (G_UNLIKELY (family))
    {
      add_attribute (tag, ns_pango_attr_family_new (family));
    }
  ...
  if (G_UNLIKELY (size))
    {
      ...
      if (parse_length (size, &n) && n > 0)
        {
          add_attribute (tag, ns_pango_attr_size_new (n));
          if (tag)
  ...
  if (G_UNLIKELY (style))
    {
      NsPangoStyle ns_pango_style;

      if (ns_pango_parse_style (style, &ns_pango_style, FALSE))
	add_attribute (tag, ns_pango_attr_style_new (ns_pango_style));
      else
	{
	  g_set_error (error,
		       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (weight))
    {
      NsPangoWeight ns_pango_weight;

      if (ns_pango_parse_weight (weight, &ns_pango_weight, FALSE))
	add_attribute (tag,
		       ns_pango_attr_weight_new (ns_pango_weight));
      else
	{
	  g_set_error (error,
		       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (variant))
    {
      NsPangoVariant ns_pango_variant;

      if (ns_pango_parse_variant (variant, &ns_pango_variant, FALSE))
	add_attribute (tag, ns_pango_attr_variant_new (ns_pango_variant));
      else
	{
	  g_set_error (error,
		       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (stretch))
    {
      NsPangoStretch ns_pango_stretch;

      if (ns_pango_parse_stretch (stretch, &ns_pango_stretch, FALSE))
	add_attribute (tag, ns_pango_attr_stretch_new (ns_pango_stretch));
      else
	{
	  g_set_error (error,
		       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (width))
    {
      NsPangoWidth ns_pango_width;

      if (ns_pango_parse_width (width, &ns_pango_width, FALSE))
        add_attribute (tag, ns_pango_attr_width_new (ns_pango_width));
      else
        {
          g_set_error (error,
                       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (foreground))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_foreground_new (color.red, color.green, color.blue));
      if (alpha != 0xffff)
        add_attribute (tag, ns_pango_attr_foreground_alpha_new (alpha));
    }
  ...
  if (G_UNLIKELY (background))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_background_new (color.red, color.green, color.blue));
      if (alpha != 0xffff)
        add_attribute (tag, ns_pango_attr_background_alpha_new (alpha));
    }
  ...
  if (G_UNLIKELY (alpha))
    {
      ...
      add_attribute (tag, ns_pango_attr_foreground_alpha_new (val));
    }
  ...
  if (G_UNLIKELY (background_alpha))
    {
      ...
      add_attribute (tag, ns_pango_attr_background_alpha_new (val));
    }

  if (G_UNLIKELY (underline))
    {
      NsPangoUnderline ul = NS_PANGO_UNDERLINE_NONE;

      if (!span_parse_enum ("underline", underline, NS_TYPE_PANGO_UNDERLINE, (int*)(void*)&ul, line_number, error))
	goto error;

      add_attribute (tag, ns_pango_attr_underline_new (ul));
    }
  ...
  if (G_UNLIKELY (underline_color))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_underline_color_new (color.red, color.green, color.blue));
    }

  if (G_UNLIKELY (overline))
    {
      NsPangoOverline ol = NS_PANGO_OVERLINE_NONE;

      if (!span_parse_enum ("overline", overline, NS_TYPE_PANGO_OVERLINE, (int*)(void*)&ol, line_number, error))
	goto error;

      add_attribute (tag, ns_pango_attr_overline_new (ol));
    }
  ...
  if (G_UNLIKELY (overline_color))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_overline_color_new (color.red, color.green, color.blue));
    }

  if (G_UNLIKELY (gravity))
    {
      NsPangoGravity gr = NS_PANGO_GRAVITY_SOUTH;

      if (!span_parse_enum ("gravity", gravity, NS_TYPE_PANGO_GRAVITY, (int*)(void*)&gr, line_number, error))
	goto error;

      if (gr == NS_PANGO_GRAVITY_AUTO)
        {
	  g_set_error (error,
		       G_MARKUP_ERROR,
		       G_MARKUP_ERROR_INVALID_CONTENT,
      ...
      add_attribute (tag, ns_pango_attr_gravity_new (gr));
    }

  if (G_UNLIKELY (gravity_hint))
    {
      NsPangoGravityHint hint = NS_PANGO_GRAVITY_HINT_NATURAL;

      if (!span_parse_enum ("gravity_hint", gravity_hint, NS_TYPE_PANGO_GRAVITY_HINT, (int*)(void*)&hint, line_number, error))
	goto error;

      add_attribute (tag, ns_pango_attr_gravity_hint_new (hint));
    }
  ...
  if (G_UNLIKELY (strikethrough))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_strikethrough_new (b));
    }
  ...
  if (G_UNLIKELY (strikethrough_color))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_strikethrough_color_new (color.red, color.green, color.blue));
    }
  ...
  if (G_UNLIKELY (fallback))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_fallback_new (b));
    }

  if (G_UNLIKELY (show))
    {
      NsPangoShowFlags flags;

      if (!span_parse_flags ("show", show, NS_TYPE_PANGO_SHOW_FLAGS, (int*)(void*)&flags, line_number, error))
	goto error;

      add_attribute (tag, ns_pango_attr_show_new (flags));
    }

  if (G_UNLIKELY (text_transform))
    {
      NsPangoTextTransform tf;

      if (!span_parse_enum ("text_transform", text_transform, NS_TYPE_PANGO_TEXT_TRANSFORM, (int*)(void*)&tf, line_number, error))
	goto error;

      add_attribute (tag, ns_pango_attr_text_transform_new (tf));
    }
  ...
  if (G_UNLIKELY (rise))
    {
      ...
      add_attribute (tag, ns_pango_attr_rise_new (n));
    }
  ...
  if (G_UNLIKELY (baseline_shift))
    {
      ...
      if (span_parse_enum ("baseline_shift", baseline_shift, NS_TYPE_PANGO_BASELINE_SHIFT, (int*)(void*)&shift, line_number, NULL))
        add_attribute (tag, ns_pango_attr_baseline_shift_new (shift));
  ...
  if (G_UNLIKELY (font_scale))
    {
      NsPangoFontScale scale;

      if (!span_parse_enum ("font_scale", font_scale, NS_TYPE_PANGO_FONT_SCALE, (int*)(void*)&scale, line_number, error))
	goto error;

      add_attribute (tag, ns_pango_attr_font_scale_new (scale));
    }
  ...
  if (G_UNLIKELY (letter_spacing))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_letter_spacing_new (n));
    }
  ...
  if (G_UNLIKELY (line_height))
    {
      ...
      if (f > 1024.0 && strchr (line_height, '.') == 0)
        add_attribute (tag, ns_pango_attr_line_height_new_absolute ((int)f));
      else
        add_attribute (tag, ns_pango_attr_line_height_new (f));
    }

  if (G_UNLIKELY (lang))
    {
      add_attribute (tag,
		     ns_pango_attr_language_new (ns_pango_language_from_string (lang)));
    }

  if (G_UNLIKELY (font_features))
    {
      add_attribute (tag, ns_pango_attr_font_features_new (font_features));
    }
  ...
  if (G_UNLIKELY (allow_breaks))
    {
      ...
      add_attribute (tag, ns_pango_attr_allow_breaks_new (b));
    }
  ...
  if (G_UNLIKELY (insert_hyphens))
	...
	goto error;
      ...
      add_attribute (tag, ns_pango_attr_insert_hyphens_new (b));
    }

  if (G_UNLIKELY (segment))
    {
      if (strcmp (segment, "word") == 0)
        add_attribute (tag, ns_pango_attr_word_new ());
      else if (strcmp (segment, "sentence") == 0)
        add_attribute (tag, ns_pango_attr_sentence_new ());
      else
        {
          g_set_error (error,
                       G_MARKUP_ERROR,
...
{
  CHECK_NO_ATTRS("i");
  add_attribute (tag, ns_pango_attr_style_new (NS_PANGO_STYLE_ITALIC));
...
{
  CHECK_NO_ATTRS("s");
  add_attribute (tag, ns_pango_attr_strikethrough_new (TRUE));
		     ...
		     GError               **error)
{
  ...
  add_attribute (tag, ns_pango_attr_font_scale_new (NS_PANGO_FONT_SCALE_SUBSCRIPT));
  add_attribute (tag, ns_pango_attr_baseline_shift_new (NS_PANGO_BASELINE_SHIFT_SUBSCRIPT));
		     ...
		     GError               **error)
{
  ...
  add_attribute (tag, ns_pango_attr_font_scale_new (NS_PANGO_FONT_SCALE_SUPERSCRIPT));
  add_attribute (tag, ns_pango_attr_baseline_shift_new (NS_PANGO_BASELINE_SHIFT_SUPERSCRIPT));
		     ...
		     GError               **error)
{
  ...
  add_attribute (tag, ns_pango_attr_family_new ("Monospace"));
...
{
  CHECK_NO_ATTRS("u");
  add_attribute (tag, ns_pango_attr_underline_new (NS_PANGO_UNDERLINE_SINGLE));
