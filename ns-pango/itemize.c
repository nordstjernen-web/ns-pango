...
static FontCache *
get_font_cache (NsPangoFontset *fontset)
{
  FontCache *cache;
...
typedef struct _PangoWidthIter NsPangoWidthIter;
...
static void
width_iter_next (NsPangoWidthIter *iter)
{
...
static void
width_iter_init (NsPangoWidthIter *iter,
                 const char     *text,
                 int             length)
{
...
static void
width_iter_fini (NsPangoWidthIter *iter)
{
}
...
struct _ItemizeState
{
  ...
  NsPangoGravity font_desc_gravity;
  gboolean centered_baseline;

  NsPangoAttrIterator *attr_iter;
  gboolean free_attr_iter;
  const char *attr_end;
  NsPangoFontDescription *font_desc;
  NsPangoFontDescription *emoji_font_desc;
  NsPangoFontDescription *text_emoji_font_desc;
  ...
  NsPangoScriptIter script_iter;
  const char *script_end;
  NsPangoScript script;

  NsPangoWidthIter width_iter;
  NsPangoEmojiIter emoji_iter;

  NsPangoLanguage *derived_lang;

  NsPangoFontset *current_fonts;
  FontCache *cache;
  NsPangoFont *base_font;
                ...
                NsPangoAttrType  type)
{
  ...
  for (node = attr_list; node; node = node->next)
    if (((NsPangoAttribute *) node->data)->klass->type == type)
      return (NsPangoAttribute *) node->data;
...
{
  NsPangoLanguage *old_lang;
  gboolean old_lang_was_explicit;
  NsPangoAttribute *attr;
  int end_index;

  ns_pango_attr_iterator_range (state->attr_iter, NULL, &end_index);
  ...
  if (state->emoji_font_desc)
    {
      ns_pango_font_description_free (state->emoji_font_desc);
      state->emoji_font_desc = NULL;
    }

  if (state->text_emoji_font_desc)
    {
      ns_pango_font_description_free (state->text_emoji_font_desc);
  ...
  if (state->font_desc)
    ns_pango_font_description_free (state->font_desc);
  state->font_desc = ns_pango_font_description_copy_static (state->context->font_desc);
  ns_pango_attr_iterator_get_font (state->attr_iter, state->font_desc,
                                &state->lang, &state->extra_attrs);
  if (ns_pango_font_description_get_set_fields (state->font_desc) & NS_PANGO_FONT_MASK_GRAVITY)
    state->font_desc_gravity = ns_pango_font_description_get_gravity (state->font_desc);
  else
    state->font_desc_gravity = NS_PANGO_GRAVITY_AUTO;
  ...
  attr = find_attribute (state->extra_attrs, NS_PANGO_ATTR_FALLBACK);
  state->enable_fallback = (attr == NULL || ((NsPangoAttrInt *)attr)->value);

  attr = find_attribute (state->extra_attrs, NS_PANGO_ATTR_GRAVITY);
  state->gravity = attr == NULL ? NS_PANGO_GRAVITY_AUTO : ((NsPangoAttrInt *)attr)->value;

  attr = find_attribute (state->extra_attrs, NS_PANGO_ATTR_GRAVITY_HINT);
  state->gravity_hint = attr == NULL ? state->context->gravity_hint : (NsPangoGravityHint)((NsPangoAttrInt *)attr)->value;
                    ...
                    const NsPangoFontDescription *desc)
{
  ...
  ns_pango_log2vis_fill_embedding_levels (text + start_index, length, n_chars, state->embedding_levels, &base_dir);

  state->embedding_end_offset = 0;
  state->embedding_end = text + start_index;
  update_embedding_end (state);

  state->gravity = NS_PANGO_GRAVITY_AUTO;
  state->centered_baseline = NS_PANGO_GRAVITY_IS_VERTICAL (state->context->resolved_gravity);
  state->gravity_hint = state->context->gravity_hint;
  state->resolved_gravity = NS_PANGO_GRAVITY_AUTO;
  ...
  if (cached_iter)
    ...
    {
      state->attr_iter = ns_pango_attr_list_get_iterator (attrs);
  ...
  if (state->attr_iter)
    {
      ...
      ns_pango_attr_iterator_advance (state->attr_iter, start_index);
      update_attr_iterator (state);
    }
  else
    {
      state->font_desc = ns_pango_font_description_copy_static (desc ? desc : state->context->font_desc);
      state->lang = state->context->language;
      state->lang_is_explicit = FALSE;
   ...
   */
  _ns_pango_script_iter_init (&state->script_iter, text + start_index, length);
  ns_pango_script_iter_get_range (&state->script_iter, NULL,
                               &state->script_end, &state->script);

  width_iter_init (&state->width_iter, text + start_index, length);
  _ns_pango_emoji_iter_init (&state->emoji_iter, text + start_index, length, n_chars);

  if (!NS_PANGO_GRAVITY_IS_VERTICAL (state->context->resolved_gravity))
...
itemize_state_next (ItemizeState *state)
{
  ...
  if (state->run_end == state->attr_end)
    {
      ns_pango_attr_iterator_next (state->attr_iter);
      update_attr_iterator (state);
    }

  if (state->run_end == state->script_end)
    {
      ns_pango_script_iter_next (&state->script_iter);
      ns_pango_script_iter_get_range (&state->script_iter, NULL,
                                   &state->script_end, &state->script);
  ...
  if (state->run_end == state->emoji_iter.end)
    {
      _ns_pango_emoji_iter_next (&state->emoji_iter);
...
copy_attr_slist (GSList *attr_slist)
{
  ...
  for (l = attr_slist; l; l = l->next)
    new_list = g_slist_prepend (new_list, ns_pango_attribute_copy (l->data));
                             ...
                             gboolean      is_space)
{
  ...
  state->item->analysis.flags |= state->centered_baseline ? NS_PANGO_ANALYSIS_FLAG_CENTERED_BASELINE : 0;
...
static gboolean
get_font_foreach (NsPangoFontset *fontset,
                  NsPangoFont    *font,
                  gpointer      data)
{
  ...
  if (ns_pango_font_has_char (font, info->wc))
    {
      info->font = font;
      return TRUE;
    }
...
{
  if (!state->base_font)
    state->base_font = ns_pango_font_map_load_font (state->context->font_map,
                                                 state->context,
                                                 state->font_desc);
          ...
          int           *position)
{
  ...
  if (state->enable_fallback)
    ns_pango_fontset_foreach (state->current_fonts, get_font_foreach, &info);
...
compute_derived_language (NsPangoLanguage *lang,
                          NsPangoScript    script)
{
  NsPangoLanguage *derived_lang;
   ...
   */
  if (lang && ns_pango_language_includes_script (lang, script))
    derived_lang = lang;
  else
    {
      derived_lang = ns_pango_script_get_sample_language (script);
      /* If we don't find a sample language for the script, we
       * use a language tag that shouldn't actually be used
       * anywhere. This keeps fontconfig (for the NsPangoFc*
       ...
       */
      if (!derived_lang)
        derived_lang = ns_pango_language_from_string ("xx");
    }
...
itemize_state_update_for_new_run (ItemizeState *state)
{
  ...
  if (state->changed & (FONT_CHANGED | SCRIPT_CHANGED | WIDTH_CHANGED))
    {
      ...
      if (state->font_desc_gravity != NS_PANGO_GRAVITY_AUTO)
        {
          state->resolved_gravity = state->font_desc_gravity;
        }
      else
        {
          NsPangoGravity gravity = state->gravity;
          NsPangoGravityHint gravity_hint = state->gravity_hint;

          if (G_LIKELY (gravity == NS_PANGO_GRAVITY_AUTO))
            gravity = state->context->resolved_gravity;

          state->resolved_gravity = ns_pango_gravity_get_for_script_and_width (state->script,
      ...
      if (state->font_desc_gravity != state->resolved_gravity)
        {
          ns_pango_font_description_set_gravity (state->font_desc, state->resolved_gravity);
          state->changed |= FONT_CHANGED;
        }
    }
  ...
  if (state->changed & (SCRIPT_CHANGED | LANG_CHANGED))
    {
      NsPangoLanguage *old_derived_lang = state->derived_lang;
  ...
  if (!state->current_fonts)
    {
      ...
      if (is_emoji && !state->emoji_font_desc)
        {
          state->emoji_font_desc = ns_pango_font_description_copy_static (state->font_desc);
          ns_pango_font_description_set_family_static (state->emoji_font_desc, "emoji");
          ns_pango_font_description_set_color (state->emoji_font_desc, has_vs ? NS_PANGO_FONT_COLOR_REQUIRED : NS_PANGO_FONT_COLOR_DONT_CARE);
        }
      if (!is_emoji && has_vs && !state->text_emoji_font_desc)
        {
          state->text_emoji_font_desc = ns_pango_font_description_copy_static (state->font_desc);
          ns_pango_font_description_set_family_static (state->text_emoji_font_desc, "emoji");
          ns_pango_font_description_set_color (state->text_emoji_font_desc, NS_PANGO_FONT_COLOR_FORBIDDEN);
        }
      state->current_fonts = ns_pango_font_map_load_fontset (state->context->font_map,
                                                          state->context,
      ...
      if (G_UNLIKELY (!get_font (state, ' ', &font, &position)))
        {
          ...
          NsPangoFontMap *fontmap = state->context->font_map;
...
itemize_state_finish (ItemizeState *state)
{
  ...
  if (state->free_attr_iter)
    ns_pango_attr_iterator_destroy (state->attr_iter);
  _ns_pango_script_iter_fini (&state->script_iter);
  ns_pango_font_description_free (state->font_desc);
  ns_pango_font_description_free (state->emoji_font_desc);
  ns_pango_font_description_free (state->text_emoji_font_desc);
  width_iter_fini (&state->width_iter);
  _ns_pango_emoji_iter_fini (&state->emoji_iter);
...
static gboolean
collect_font_scale (NsPangoContext  *context,
                    GList        **stack,
                    NsPangoItem     *item,
                    NsPangoItem     *prev,
                    double        *scale,
  ...
  for (GSList *l = item->analysis.extra_attrs; l; l = l->next)
    {
      ...
      if (attr->klass->type == NS_PANGO_ATTR_FONT_SCALE)
        {
          if (attr->start_index == item->offset)
            {
              ...
              *stack = g_list_prepend (*stack, entry);
              ...
              switch (((NsPangoAttrInt *)attr)->value)
                {
                case NS_PANGO_FONT_SCALE_NONE:
                  break;
                case NS_PANGO_FONT_SCALE_SUPERSCRIPT:
                  if (prev &&
                      prev->analysis.font &&
                      hb_ot_metrics_get_position (ns_pango_font_get_hb_font (prev->analysis.font),
                                                  HB_OT_METRICS_TAG_SUPERSCRIPT_EM_Y_SIZE,
                                                  &y_size))
                    {
                      hb_font_get_scale (ns_pango_font_get_hb_font (prev->analysis.font), NULL, &y_scale);
                  ...
                  break;
                case NS_PANGO_FONT_SCALE_SUBSCRIPT:
                  if (prev &&
                      prev->analysis.font &&
                      hb_ot_metrics_get_position (ns_pango_font_get_hb_font (prev->analysis.font),
                  ...
                  break;
                case NS_PANGO_FONT_SCALE_SMALL_CAPS:
                  if (item->analysis.font &&
                      hb_ot_metrics_get_position (ns_pango_font_get_hb_font (item->analysis.font),
   ...
   for (l = *stack; l; l = l->next)
     {
       ...
       *scale *= entry->scale;
       if (((NsPangoAttrInt *)entry->attr)->value != NS_PANGO_FONT_SCALE_SMALL_CAPS)
         *is_small_caps = FALSE;
       retval = TRUE;
     }
...
static void
apply_scale_to_item (NsPangoContext *context,
                     NsPangoItem    *item,
                     double        scale,
                     gboolean      is_small_caps)
{
  ...
  if (is_small_caps)
    ns_pango_analysis_set_size_font (&item->analysis, item->analysis.font);

  desc = ns_pango_font_describe (item->analysis.font);
  size = scale * ns_pango_font_description_get_size (desc);
  ...
  item->analysis.font = ns_pango_font_map_load_font (context->font_map, context, desc);

  ns_pango_font_description_free (desc);
}

static void
apply_font_scale (NsPangoContext *context,
                  GList        *items)
{
...
static gboolean
all_features_supported (NsPangoItem *item,
                        hb_tag_t  *features,
                        guint      n_features)
{
  hb_font_t *font = ns_pango_font_get_hb_font (item->analysis.font);
...
static gboolean
variant_supported (NsPangoItem    *item,
...
get_font_variant (NsPangoItem *item)
{
  if (item->analysis.font)
    return ns_pango_font_get_variant (item->analysis.font);
...
split_item_for_variant (const char   *text,
                        ...
                        GList        *list_item)
{
  NsPangoItem *item = list_item->data;
  const char *start, *end;
  const char *p, *p0;
  gunichar wc;
  NsPangoTextTransform transform = NS_PANGO_TEXT_TRANSFORM_NONE;
  NsPangoFontScale lowercase_scale = NS_PANGO_FONT_SCALE_NONE;
  NsPangoFontScale uppercase_scale = NS_PANGO_FONT_SCALE_NONE;
  NsPangoTextTransform item_transform;
  ...
  switch (variant)
    ...
    case NS_PANGO_VARIANT_ALL_SMALL_CAPS:
    case NS_PANGO_VARIANT_ALL_PETITE_CAPS:
      uppercase_scale = NS_PANGO_FONT_SCALE_SMALL_CAPS;
      G_GNUC_FALLTHROUGH;
    case NS_PANGO_VARIANT_SMALL_CAPS:
    case NS_PANGO_VARIANT_PETITE_CAPS:
      transform = NS_PANGO_TEXT_TRANSFORM_UPPERCASE;
      lowercase_scale = NS_PANGO_FONT_SCALE_SMALL_CAPS;
      break;
    case NS_PANGO_VARIANT_UNICASE:
      uppercase_scale = NS_PANGO_FONT_SCALE_SMALL_CAPS;
      break;
  ...
  offset = ((NsPangoItemPrivate *)item)->char_offset;
  ...
  while (p < end)
    {
      ...
      while (p < end && (item_transform == NS_PANGO_TEXT_TRANSFORM_LOWERCASE ||
                         consider_as_space (wc) ||
                         (g_unichar_islower (wc) &&
                          !(item_transform == NS_PANGO_TEXT_TRANSFORM_UPPERCASE ||
                            (item_transform == NS_PANGO_TEXT_TRANSFORM_CAPITALIZE && is_word_start)))))
        {
          p = g_utf8_next_char (p);
          wc = g_utf8_get_char (p);
          offset++;
      ...
      if (p0 < p)
        {
          ...
          if (p < end)
            {
              new_item = ns_pango_item_split (item, p - p0, g_utf8_strlen (p0, p - p0));
          ...
          if (transform != NS_PANGO_TEXT_TRANSFORM_NONE)
            {
              attr = ns_pango_attr_text_transform_new (transform);
          ...
          if (lowercase_scale != NS_PANGO_FONT_SCALE_NONE)
            {
              attr = ns_pango_attr_font_scale_new (lowercase_scale);
              attr->start_index = new_item->offset;
              attr->end_index = new_item->offset + new_item->length;
              new_item->analysis.extra_attrs = g_slist_append (new_item->analysis.extra_attrs, attr);
            }
      ...
      wc = g_utf8_get_char (p);
      is_word_start = log_attrs && log_attrs[offset].is_word_start;
      while (p < end && (item_transform == NS_PANGO_TEXT_TRANSFORM_UPPERCASE ||
                         consider_as_space (wc) ||
                         !(item_transform == NS_PANGO_TEXT_TRANSFORM_LOWERCASE || g_unichar_islower (wc)) ||
                         (item_transform == NS_PANGO_TEXT_TRANSFORM_CAPITALIZE && is_word_start)))
        {
          p = g_utf8_next_char (p);
          wc = g_utf8_get_char (p);
      ...
      if (p0 < p)
        {
          ...
          if (uppercase_scale != NS_PANGO_FONT_SCALE_NONE)
            {
              attr = ns_pango_attr_font_scale_new (uppercase_scale);
              attr->start_index = new_item->offset;
              attr->end_index = new_item->offset + new_item->length;
              new_item->analysis.extra_attrs = g_slist_append (new_item->analysis.extra_attrs, attr);
            }
...
static GList *
reorder_items (NsPangoContext *context,
               GList        *items,
               int           initial_offset)
{
  ...
  for (GList *l = items; l; l = l->next)
    {
      NsPangoItemPrivate *item = l->data;
...
static GList *
post_process_items (NsPangoContext *context,
                    const char   *text,
                    NsPangoLogAttr *log_attrs,
                    GList        *items)
{
...
/* Like ns_pango_itemize_with_base_dir, but takes a font description.
 * In contrast to ns_pango_itemize_with_base_dir, this function does
 * not call ns_pango_itemize_post_process_items, so you need to do that
 * separately, after applying attributes that affect segmentation and
 * computing the log attrs.
 */
GList *
ns_pango_itemize_with_font (NsPangoContext               *context,
                         NsPangoDirection              base_dir,
                         const char                 *text,
                         int                         start_index,
                         int                         length,
 ...
 */
GList *
ns_pango_itemize_post_process_items (NsPangoContext *context,
                                  const char   *text,
                                  NsPangoLogAttr *log_attrs,
                                  GList        *items)
{
...
/**
 * ns_pango_itemize_with_base_dir:
 ...
 *
 * Like `ns_pango_itemize()`, but with an explicitly specified base direction.
 *
 * The base direction is used when computing bidirectional levels.
 * [func@itemize] gets the base direction from the `NsPangoContext`
 ...
 */
GList *
ns_pango_itemize_with_base_dir (NsPangoContext      *context,
                             NsPangoDirection     base_dir,
                             const char        *text,
                             int                start_index,
                             int                length,
                             ...
                             NsPangoAttrIterator *cached_iter)
{
  ...
  items = ns_pango_itemize_with_font (context, base_dir,
                                   text, start_index, length,
                                   attrs, cached_iter,
                                   NULL);

  return ns_pango_itemize_post_process_items (context, text, NULL, items);
}
 ...
 */
GList *
ns_pango_itemize (NsPangoContext      *context,
               const char        *text,
               int                start_index,
               int                length,
               NsPangoAttrList     *attrs,
               NsPangoAttrIterator *cached_iter)
{
  ...
  return ns_pango_itemize_with_base_dir (context, context->base_dir,
                                      text, start_index, length,
                                      attrs, cached_iter);
}
