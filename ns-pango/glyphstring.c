...
/**
 * ns_pango_glyph_string_new:
 *
 * Create a new `NsPangoGlyphString`.
 *
 * Return value: the newly allocated `NsPangoGlyphString`, which
 *   should be freed with [method@Pango.GlyphString.free].
 */
NsPangoGlyphString *
ns_pango_glyph_string_new (void)
{
  NsPangoGlyphString *string = g_slice_new (NsPangoGlyphString);
...
/**
 * ns_pango_glyph_string_set_size:
 * @string: a `NsPangoGlyphString`.
 * @new_len: the new length of the string
 *
 * Resize a glyph string to the given length.
 */
void
ns_pango_glyph_string_set_size (NsPangoGlyphString *string, gint new_len)
{
  ...
  while (new_len > string->space)
      ...
      else
	{
	  const guint max_space =
	    MIN (G_MAXINT, G_MAXSIZE / MAX (sizeof(NsPangoGlyphInfo), sizeof(gint)));
  ...
  string->glyphs = g_realloc (string->glyphs, string->space * sizeof (NsPangoGlyphInfo));
  string->log_clusters = g_realloc (string->log_clusters, string->space * sizeof (gint));
  string->num_glyphs = new_len;
}

G_DEFINE_BOXED_TYPE (NsPangoGlyphString, ns_pango_glyph_string,
                     ns_pango_glyph_string_copy,
                     ns_pango_glyph_string_free);

/**
 * ns_pango_glyph_string_copy:
 * @string: (nullable): a `NsPangoGlyphString`
 *
 * Copy a glyph string and associated storage.
 *
 * Return value: (nullable): the newly allocated `NsPangoGlyphString`
 */
NsPangoGlyphString *
ns_pango_glyph_string_copy (NsPangoGlyphString *string)
{
  NsPangoGlyphString *new_string;

  if (string == NULL)
    return NULL;

  new_string = g_slice_new (NsPangoGlyphString);

  *new_string = *string;

  new_string->glyphs = g_memdup2 (string->glyphs,
                                  string->space * sizeof (NsPangoGlyphInfo));
...
/**
 * ns_pango_glyph_string_free:
 * @string: (nullable): a `NsPangoGlyphString`, may be %NULL
 *
 * Free a glyph string and associated storage.
 */
void
ns_pango_glyph_string_free (NsPangoGlyphString *string)
{
  if (string == NULL)
    return;
  ...
  g_slice_free (NsPangoGlyphString, string);
}

/**
 * ns_pango_glyph_string_extents_range:
 ...
 */
void
ns_pango_glyph_string_extents_range (NsPangoGlyphString *glyphs,
				  int               start,
				  int               end,
				  NsPangoFont        *font,
				  ...
				  NsPangoRectangle   *logical_rect)
{
  ...
  for (i = start; i < end; i++)
    {
      NsPangoRectangle glyph_ink;
      NsPangoRectangle glyph_logical;

      NsPangoGlyphGeometry *geometry = &glyphs->glyphs[i].geometry;

      ns_pango_font_get_glyph_extents (font, glyphs->glyphs[i].glyph,
 ...
 */
void
ns_pango_glyph_string_extents (NsPangoGlyphString *glyphs,
			    NsPangoFont        *font,
			    NsPangoRectangle   *ink_rect,
			    NsPangoRectangle   *logical_rect)
{
  ns_pango_glyph_string_extents_range (glyphs, 0, glyphs->num_glyphs,
				    font, ink_rect, logical_rect);
}

/**
 * ns_pango_glyph_string_get_width:
 * @glyphs:  a `NsPangoGlyphString`
 *
 ...
 */
int
ns_pango_glyph_string_get_width (NsPangoGlyphString *glyphs)
{
  int i;
  int width = 0;
...
/**
 * ns_pango_glyph_string_get_logical_widths:
 * @glyphs: a `NsPangoGlyphString`
 ...
 *   widths.
 *
 * Given a `NsPangoGlyphString` and corresponding text, determine the width
 ...
 */
void
ns_pango_glyph_string_get_logical_widths (NsPangoGlyphString *glyphs,
				       const char       *text,
				       int               length,
				       int               embedding_level,
				       int              *logical_widths)
{
  /* Build a NsPangoGlyphItem and call the other API */
  NsPangoItem item = {0, length, ns_pango_utf8_strlen (text, length),
		    {NULL, NULL, NULL,
		     embedding_level, NS_PANGO_GRAVITY_AUTO, 0,
		     NS_PANGO_SCRIPT_UNKNOWN, NULL,
		     NULL}};
  NsPangoGlyphItem glyph_item = {&item, glyphs};

  ns_pango_glyph_item_get_logical_widths (&glyph_item, text, logical_widths);
}
...
/**
 * ns_pango_glyph_string_index_to_x:
 ...
 */
void
ns_pango_glyph_string_index_to_x (NsPangoGlyphString *glyphs,
                               const char       *text,
                               int               length,
                               ...
                               int              *x_pos)
{
  ns_pango_glyph_string_index_to_x_full (glyphs,
                                      text, length,
                                      analysis,
                                      NULL,
                                      index, trailing,
...
/**
 * ns_pango_glyph_string_index_to_x_full:
 * @glyphs: the glyphs return from [func@shape]
 * @text: the text for the run
 * @length: the number of bytes (not characters) in @text.
 * @analysis: the analysis information return from [func@itemize]
 * @attrs: (nullable): `NsPangoLogAttr` array for @text
 * @index_: the byte index within @text
 * @trailing: whether we should compute the result for the beginning (%FALSE)
 *   or end (%TRUE) of the character.
 * @x_pos: (out) (optional): location to store result
 ...
 * accepts a `NsPangoLogAttr` array. The grapheme boundary information
 ...
 */
void
ns_pango_glyph_string_index_to_x_full (NsPangoGlyphString *glyphs,
                                    const char       *text,
                                    int               length,
                                    NsPangoAnalysis    *analysis,
...
/**
 * ns_pango_glyph_string_x_to_index:
 ...
 */
void
ns_pango_glyph_string_x_to_index (NsPangoGlyphString *glyphs,
                               const char       *text,
                               int               length,
                               NsPangoAnalysis    *analysis,
                               int               x_pos,
