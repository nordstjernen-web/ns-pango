...
typedef struct _PangoCairoContextInfo NsPangoCairoContextInfo;
...
struct _PangoCairoContextInfo
{
  ...
  NsPangoCairoShapeRendererFunc shape_renderer_func;
  gpointer                    shape_renderer_data;
  GDestroyNotify              shape_renderer_notify;
};

static void
free_context_info (NsPangoCairoContextInfo *info)
{
  if (info->set_options)
  ...
  g_slice_free (NsPangoCairoContextInfo, info);
}

static NsPangoCairoContextInfo *
get_context_info (NsPangoContext *context,
		  gboolean      create)
{
  static GQuark context_info_quark; /* MT-safe */
  NsPangoCairoContextInfo *info;
...
retry:
  ...
  if (G_UNLIKELY (!info) && create)
    {
      info = g_slice_new0 (NsPangoCairoContextInfo);
      info->dpi = -1.0;
...
static void
_ns_pango_cairo_update_context (cairo_t      *cr,
			     NsPangoContext *context)
{
  NsPangoCairoContextInfo *info;
  cairo_matrix_t cairo_matrix;
  cairo_surface_t *target;
  NsPangoMatrix ns_pango_matrix;
  const NsPangoMatrix *current_matrix, identity_matrix = NS_PANGO_MATRIX_INIT;
  ...
  merged_options = _ns_pango_cairo_context_get_merged_font_options (context);
  ...
  ns_pango_matrix.xx = cairo_matrix.xx;
  ns_pango_matrix.yx = cairo_matrix.yx;
  ns_pango_matrix.xy = cairo_matrix.xy;
  ns_pango_matrix.yy = cairo_matrix.yy;
  ns_pango_matrix.x0 = 0;
  ns_pango_matrix.y0 = 0;

  current_matrix = ns_pango_context_get_matrix (context);
  if (!current_matrix)
  ...
  if ((cairo_font_options_get_hint_metrics (merged_options) != CAIRO_HINT_METRICS_OFF) &&
      (0 != memcmp (&ns_pango_matrix, current_matrix, sizeof (NsPangoMatrix))))
    changed = TRUE;

  ns_pango_context_set_matrix (context, &ns_pango_matrix);

  if (changed)
    ns_pango_context_changed (context);
}

/**
 * ns_pango_cairo_update_context:
 * @cr: a Cairo context
 * @context: a `NsPangoContext`, from a pangocairo font map
 *
 * Updates a `NsPangoContext` previously created for use with Cairo to
 ...
 */
void
ns_pango_cairo_update_context (cairo_t      *cr,
			    NsPangoContext *context)
{
  g_return_if_fail (cr != NULL);
  g_return_if_fail (NS_PANGO_IS_CONTEXT (context));

  _ns_pango_cairo_update_context (cr, context);
}

/**
 * ns_pango_cairo_context_set_resolution:
 * @context: a `NsPangoContext`, from a pangocairo font map
 ...
 *
 * This is a scale factor between points specified in a `NsPangoFontDescription`
 ...
 */
void
ns_pango_cairo_context_set_resolution (NsPangoContext *context,
				    double        dpi)
{
  NsPangoCairoContextInfo *info = get_context_info (context, TRUE);
  info->dpi = dpi;
}

/**
 * ns_pango_cairo_context_get_resolution:
 * @context: a `NsPangoContext`, from a pangocairo font map
 *
 * Gets the resolution for the context.
 *
 * See [func@NsPangoCairo.context_set_resolution]
 *
 ...
 */
double
ns_pango_cairo_context_get_resolution (NsPangoContext *context)
{
  NsPangoCairoContextInfo *info = get_context_info (context, FALSE);
...
/**
 * ns_pango_cairo_context_set_font_options:
 * @context: a `NsPangoContext`, from a pangocairo font map
 ...
 */
void
ns_pango_cairo_context_set_font_options (NsPangoContext               *context,
...
/**
 * ns_pango_cairo_context_get_font_options:
 * @context: a `NsPangoContext`, from a pangocairo font map
 *
 * Retrieves any font rendering options previously set with
 * [func@NsPangoCairo.context_set_font_options].
 *
...
ns_pango_cairo_context_get_font_options (NsPangoContext *context)
{
  NsPangoCairoContextInfo *info;
...
/**
 * _ns_pango_cairo_context_merge_font_options:
 ...
 */
const cairo_font_options_t *
_ns_pango_cairo_context_get_merged_font_options (NsPangoContext *context)
{
  NsPangoCairoContextInfo *info = get_context_info (context, TRUE);
...
/**
 * ns_pango_cairo_context_set_shape_renderer:
 * @context: a `NsPangoContext`, from a pangocairo font map
 * @func: (nullable): Callback function for rendering attributes of
 *   type %NS_PANGO_ATTR_SHAPE, or %NULL to disable shape rendering.
 ...
 * of type %NS_PANGO_ATTR_SHAPE.
 *
 * See `NsPangoCairoShapeRendererFunc` for details.
 *
 * Since: 1.18
 */
void
ns_pango_cairo_context_set_shape_renderer (NsPangoContext                *context,
...
/**
 * ns_pango_cairo_context_get_shape_renderer: (skip)
 * @context: a `NsPangoContext`, from a pangocairo font map
 ...
 * attributes of type %NS_PANGO_ATTR_SHAPE as set by
 * [func@NsPangoCairo.context_set_shape_renderer], if any.
 *
...
ns_pango_cairo_context_get_shape_renderer (NsPangoContext *context,
                                        gpointer     *data)
{
  NsPangoCairoContextInfo *info;
 ...
 * create a layout for use with @cr and do not need to access `NsPangoContext`
 * directly, you can use [func@create_layout] instead.
 *
 * Return value: (transfer full): the newly created `NsPangoContext`
 *
 * Since: 1.22
 */
NsPangoContext *
ns_pango_cairo_create_context (cairo_t *cr)
{
  ...
  fontmap = ns_pango_cairo_font_map_get_default ();
  context = ns_pango_font_map_create_context (fontmap);
  ns_pango_cairo_update_context (cr, context);

  return context;
}

/**
 * ns_pango_cairo_create_layout:
 * @cr: a Cairo context
 *
 ...
 * `NsPangoContext` object for each layout. This might matter in an
 * application that was laying out large amounts of text.
 *
 * Return value: (transfer full): the newly created `NsPangoLayout`
 *
 * Since: 1.10
 */
NsPangoLayout *
ns_pango_cairo_create_layout (cairo_t *cr)
{
  NsPangoContext *context;
  ...
  context = ns_pango_cairo_create_context (cr);
...
/**
 * ns_pango_cairo_update_layout:
 * @cr: a Cairo context
 * @layout: a `NsPangoLayout`, from [func@create_layout]
 *
 * Updates the private `NsPangoContext` of a `NsPangoLayout` created with
 ...
 */
void
ns_pango_cairo_update_layout (cairo_t     *cr,
			   NsPangoLayout *layout)
{
  g_return_if_fail (cr != NULL);
  g_return_if_fail (NS_PANGO_IS_LAYOUT (layout));

  _ns_pango_cairo_update_context (cr, ns_pango_layout_get_context (layout));
}
