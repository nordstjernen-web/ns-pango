 ...
 *
 * Implementation of ns_pango_emoji_iter is based on Chromium's Ragel-based
 * parser:
 *
...
static inline gboolean \
_ns_pango_Is_##name (gunichar ch) \
{ \
  return ch >= _ns_pango_##name##_table[0].start && \
         bsearch_interval (ch, \
                           _ns_pango_##name##_table, \
                           G_N_ELEMENTS (_ns_pango_##name##_table)); \
}
...
{
	return _ns_pango_Is_Emoji (ch);
}

gboolean
_ns_pango_Is_Emoji_Extended_Pictographic (gunichar ch)
{
	return _ns_pango_Is_Extended_Pictographic (ch);
}

static inline gboolean
_ns_pango_Is_Emoji_Emoji_Default (gunichar ch)
{
  return _ns_pango_Is_Emoji_Presentation (ch);
}

static inline gboolean
_ns_pango_Is_Emoji_Keycap_Base (gunichar ch)
{
  return (ch >= '0' && ch <= '9') || ch == '#' || ch == '*';
}

static inline gboolean
_ns_pango_Is_Regional_Indicator (gunichar ch)
{
...
enum NsPangoEmojiScannerCategory {
  EMOJI = 0,
...
_ns_pango_EmojiSegmentationCategory (gunichar codepoint)
{
  /* Specific ones first. */
  ...
  if (_ns_pango_Is_Emoji_Modifier_Base (codepoint))
    return EMOJI_MODIFIER_BASE;
  if (_ns_pango_Is_Emoji_Modifier (codepoint))
    return EMOJI_MODIFIER;
  if (_ns_pango_Is_Regional_Indicator (codepoint))
    return REGIONAL_INDICATOR;
  if (_ns_pango_Is_Emoji_Keycap_Base (codepoint))
    return KEYCAP_BASE;
  if (_ns_pango_Is_Emoji_Emoji_Default (codepoint))
    return EMOJI_EMOJI_PRESENTATION;
  if (_ns_pango_Is_Emoji (codepoint))
                        ...
                        unsigned int    n_chars)
{
  ...
  for (i = 0; i < n_chars; i++)
  {
    types[i] = _ns_pango_EmojiSegmentationCategory (g_utf8_get_char (p));
