...
typedef struct _PangoFT2RendererClass NsPangoFT2RendererClass;

#define NS_PANGO_FT2_RENDERER_CLASS(klass)    (G_TYPE_CHECK_CLASS_CAST ((klass), NS_TYPE_PANGO_FT2_RENDERER, NsPangoFT2RendererClass))
#define NS_PANGO_IS_FT2_RENDERER_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), NS_TYPE_PANGO_FT2_RENDERER))
#define NS_PANGO_FT2_RENDERER_GET_CLASS(obj)  (G_TYPE_INSTANCE_GET_CLASS ((obj), NS_TYPE_PANGO_FT2_RENDERER, NsPangoFT2RendererClass))
...
static void ns_pango_ft2_renderer_draw_glyph     (NsPangoRenderer    *renderer,
					       NsPangoFont        *font,
					       NsPangoGlyph        glyph,
					       double            x,
					       double            y);
static void ns_pango_ft2_renderer_draw_trapezoid (NsPangoRenderer    *renderer,
...
G_DEFINE_TYPE (NsPangoFT2Renderer, ns_pango_ft2_renderer, NS_TYPE_PANGO_RENDERER)

static void
ns_pango_ft2_renderer_init (NsPangoFT2Renderer *renderer G_GNUC_UNUSED)
{
}

static void
ns_pango_ft2_renderer_class_init (NsPangoFT2RendererClass *klass)
{
  NsPangoRendererClass *renderer_class = NS_PANGO_RENDERER_CLASS (klass);

  renderer_class->draw_glyph = ns_pango_ft2_renderer_draw_glyph;
  renderer_class->draw_trapezoid = ns_pango_ft2_renderer_draw_trapezoid;
}

static void
ns_pango_ft2_renderer_set_bitmap (NsPangoFT2Renderer *renderer,
			      FT_Bitmap         *bitmap)
{
...
typedef struct
{
  ...
  int bitmap_left;
  int bitmap_top;
} NsPangoFT2RenderedGlyph;

static void
ns_pango_ft2_free_rendered_glyph (NsPangoFT2RenderedGlyph *rendered)
{
  g_free (rendered->bitmap.buffer);
  g_slice_free (NsPangoFT2RenderedGlyph, rendered);
}

static NsPangoFT2RenderedGlyph *
ns_pango_ft2_font_render_box_glyph (int      width,
				 int      height,
				 int      top,
				 gboolean invalid)
{
  NsPangoFT2RenderedGlyph *box;
  ...
  box = g_slice_new (NsPangoFT2RenderedGlyph);
  ...
  if (G_UNLIKELY (!box->bitmap.buffer)) {
    g_slice_free (NsPangoFT2RenderedGlyph, box);
    return NULL;
  }
  ...
  if (invalid)
    {
      ...
      int inc = NS_PANGO_SCALE * MAX (width - line_width, 0) / (height + 1);
      offset1 = NS_PANGO_SCALE;
      offset2 = NS_PANGO_SCALE * MAX (width - line_width - 1, 0) ;
      for (i = box->bitmap.pitch;
	   i < (int) (box->bitmap.rows - 1) * box->bitmap.pitch;
	   i += box->bitmap.pitch)
        {
	  for (j = 0; j < line_width; j++)
	    {
	      box->bitmap.buffer[NS_PANGO_PIXELS (offset1) + i + j] = 0xff;
	      box->bitmap.buffer[NS_PANGO_PIXELS (offset2) + i + j] = 0xff;
	    }
	  offset1 += inc;
	  offset2 -= inc;
	}
...
static NsPangoFT2RenderedGlyph *
ns_pango_ft2_font_render_glyph (NsPangoFont *font,
			     NsPangoGlyph glyph_index)
{
  FT_Face face;
  gboolean invalid_input;

  invalid_input = glyph_index == NS_PANGO_GLYPH_INVALID_INPUT || (glyph_index & ~NS_PANGO_GLYPH_UNKNOWN_FLAG) > 0x10FFFF;

  if (glyph_index & NS_PANGO_GLYPH_UNKNOWN_FLAG)
    {
      NsPangoFT2RenderedGlyph *box;
	...
	goto generic_box;
      ...
      metrics = ns_pango_font_get_metrics (font, NULL);
      if (!metrics)
	goto generic_box;

      box = ns_pango_ft2_font_render_box_glyph (NS_PANGO_PIXELS (metrics->approximate_char_width),
					     NS_PANGO_PIXELS (metrics->ascent + metrics->descent),
					     NS_PANGO_PIXELS (metrics->ascent),
					     invalid_input);
  ...
  face = ns_pango_ft2_font_get_face (font);

  if (face)
    {
      NsPangoFT2RenderedGlyph *rendered;
      NsPangoFT2Font *ft2font = (NsPangoFT2Font *) font;

      rendered = g_slice_new (NsPangoFT2RenderedGlyph);
    ...
    {
generic_box:
      return  ns_pango_ft2_font_render_box_glyph (NS_PANGO_UNKNOWN_GLYPH_WIDTH,
					       NS_PANGO_UNKNOWN_GLYPH_HEIGHT,
					       NS_PANGO_UNKNOWN_GLYPH_HEIGHT,
					       invalid_input);
    }
}
...
static void
ns_pango_ft2_renderer_draw_glyph (NsPangoRenderer *renderer,
			       NsPangoFont     *font,
			       NsPangoGlyph     glyph,
			       double         x,
			       double         y)
{
  FT_Bitmap *bitmap = NS_PANGO_FT2_RENDERER (renderer)->bitmap;
  NsPangoFT2RenderedGlyph *rendered_glyph;
  ...
  if (glyph & NS_PANGO_GLYPH_UNKNOWN_FLAG)
    {
      ...
      gunichar wc = glyph & (~NS_PANGO_GLYPH_UNKNOWN_FLAG);

      if (G_UNLIKELY (glyph == NS_PANGO_GLYPH_INVALID_INPUT || wc > 0x10FFFF))
	glyph = NS_PANGO_GLYPH_INVALID_INPUT;
      else
	glyph = NS_PANGO_GLYPH_UNKNOWN_FLAG;
    }

  rendered_glyph = _ns_pango_ft2_font_get_cache_glyph_data (font, glyph);
  add_glyph_to_cache = FALSE;
  if (rendered_glyph == NULL)
    {
      rendered_glyph = ns_pango_ft2_font_render_glyph (font, glyph);
      if (rendered_glyph == NULL)
        return;
  ...
  if (add_glyph_to_cache)
    {
      _ns_pango_ft2_font_set_glyph_cache_destroy (font,
					       (GDestroyNotify) ns_pango_ft2_free_rendered_glyph);
      _ns_pango_ft2_font_set_cache_glyph_data (font,
...
static void
draw_simple_trap (NsPangoRenderer *renderer,
		  Position      *t,
		  Position      *b)
{
  FT_Bitmap *bitmap = NS_PANGO_FT2_RENDERER (renderer)->bitmap;
 ...
 */
static void
ns_pango_ft2_renderer_draw_trapezoid (NsPangoRenderer   *renderer,
				   NsPangoRenderPart  part G_GNUC_UNUSED,
				   double           y1,
				   double           x11,
				   double           x21,
				   double           y2,
...
/**
 * ns_pango_ft2_render_layout_subpixel:
 ...
 *
 * Render a `NsPangoLayout` onto a FreeType2 bitmap, with he
 ...
 */
void
ns_pango_ft2_render_layout_subpixel (FT_Bitmap   *bitmap,
				  ...
				  int          y)
{
  ...
  context = ns_pango_layout_get_context (layout);
  fontmap = ns_pango_context_get_font_map (context);
  renderer = _ns_pango_ft2_font_map_get_renderer (NS_PANGO_FT2_FONT_MAP (fontmap));

  ns_pango_ft2_renderer_set_bitmap (NS_PANGO_FT2_RENDERER (renderer), bitmap);

  ns_pango_renderer_draw_layout (renderer, layout, x, y);
}

/**
 * ns_pango_ft2_render_layout:
 * @bitmap: a FT_Bitmap to render the layout onto
 * @layout: a `NsPangoLayout`
 * @x: the X position of the left of the layout (in pixels)
 * @y: the Y position of the top of the layout (in pixels)
 *
 * Render a `NsPangoLayout` onto a FreeType2 bitmap
 */
void
ns_pango_ft2_render_layout (FT_Bitmap   *bitmap,
			 NsPangoLayout *layout,
			 int          x,
			 int          y)
{
  ns_pango_ft2_render_layout_subpixel (bitmap, layout, x * NS_PANGO_SCALE, y * NS_PANGO_SCALE);
}

/**
 * ns_pango_ft2_render_layout_line_subpixel:
 ...
 *
 * Render a `NsPangoLayoutLine` onto a FreeType2 bitmap, with he
 * location specified in fixed-point Pango units rather than
 * pixels.
 *
 * (Using this will avoid extra inaccuracies from rounding
 ...
 */
void
ns_pango_ft2_render_layout_line_subpixel (FT_Bitmap       *bitmap,
				       ...
				       int              y)
{
  ...
  context = ns_pango_layout_get_context (line->layout);
  fontmap = ns_pango_context_get_font_map (context);
  renderer = _ns_pango_ft2_font_map_get_renderer (NS_PANGO_FT2_FONT_MAP (fontmap));

  ns_pango_ft2_renderer_set_bitmap (NS_PANGO_FT2_RENDERER (renderer), bitmap);

  ns_pango_renderer_draw_layout_line (renderer, line, x, y);
}

/**
 * ns_pango_ft2_render_layout_line:
 * @bitmap: a FT_Bitmap to render the line onto
 * @line: a `NsPangoLayoutLine`
 * @x: the x position of start of string (in pixels)
 * @y: the y position of baseline (in pixels)
 *
 * Render a `NsPangoLayoutLine` onto a FreeType2 bitmap
 */
void
ns_pango_ft2_render_layout_line (FT_Bitmap       *bitmap,
			      NsPangoLayoutLine *line,
			      int              x,
			      int              y)
{
  ns_pango_ft2_render_layout_line_subpixel (bitmap, line, x * NS_PANGO_SCALE, y * NS_PANGO_SCALE);
}

/**
 * ns_pango_ft2_render_transformed:
 * @bitmap: the FreeType2 bitmap onto which to draw the string
 * @font: the font in which to draw the string
 * @matrix: (nullable): a `NsPangoMatrix`
 ...
 *
 * Renders a `NsPangoGlyphString` onto a FreeType2 bitmap, possibly
 * transforming the layed-out coordinates through a transformation
 * matrix.
 *
 * Note that the transformation matrix for @font is not
 * changed, so to produce correct rendering results, the @font
 * must have been loaded using a `NsPangoContext` with an identical
 * transformation matrix to that passed in to this function.
 *
 * Since: 1.6
 */
void
ns_pango_ft2_render_transformed (FT_Bitmap         *bitmap,
			      ...
			      int                y)
{
  ...
  g_return_if_fail (NS_PANGO_FT2_IS_FONT (font));

  fontmap = NS_PANGO_FC_FONT (font)->fontmap;
  renderer = _ns_pango_ft2_font_map_get_renderer (NS_PANGO_FT2_FONT_MAP (fontmap));

  ns_pango_ft2_renderer_set_bitmap (NS_PANGO_FT2_RENDERER (renderer), bitmap);
  ns_pango_renderer_set_matrix (renderer, matrix);

  ns_pango_renderer_draw_glyphs (renderer, font, glyphs, x, y);
}
 ...
 *
 * Renders a `NsPangoGlyphString` onto a FreeType2 bitmap.
 */
void
ns_pango_ft2_render (FT_Bitmap        *bitmap,
		  NsPangoFont        *font,
		  NsPangoGlyphString *glyphs,
		  int               x,
		  int               y)
{
  ns_pango_ft2_render_transformed (bitmap, NULL, font, glyphs, x * NS_PANGO_SCALE, y * NS_PANGO_SCALE);
}
